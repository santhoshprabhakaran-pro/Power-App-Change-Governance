<#
.SYNOPSIS
    Add required columns to cgmp_projects in Dataverse and export the CgmpOptionSets solution.

.DESCRIPTION
    Creates 13 new Single Line of Text columns on the cgmp_projects entity,
    adds each column to the CgmpOptionSets solution, publishes the entity,
    then exports and unpacks the solution for source control.
    ...
    Authentication is attempted in order:
      ...
      3. Browser sign-in       (Authorization Code + PKCE — opens the system browser;
                                no device code used, works in all enterprise tenants)
...
.EXAMPLE
    .\scripts\add-columns.ps1
    .\scripts\add-columns.ps1 -SkipExport
    .\scripts\add-columns.ps1 -Port 9000
#>
...
$OrgId     = '231c6d46-5369-f111-9bb3-000d3a5cc318'
$ApiBase   = "$OrgUrl/api/data/v9.2"
$Solution  = 'CgmpOptionSets'
# Logical name (singular) — used by the metadata API (EntityDefinitions).
# The data API uses the entity set name cgmp_projects (plural), but the
# metadata API requires the logical name cgmp_project (singular).
$Entity    = 'cgmp_project'

# Power Apps first-party public client — localhost redirect URIs are always
# permitted for public clients when using PKCE (per AAD v2.0 specification).
$ClientId  = '51f81489-12ee-4a9e-aaae-a2591f45987d'

# ── Columns to create ────────────────────────────────────────────────────
# All Single Line of Text. Dropdown values are managed in app code only —
# no Dataverse Choice columns are needed.
$Columns = @(
    @{ LogicalName='cgmp_pidbnumber';            DisplayName='PIDB No.';                      MaxLength=100;  Required='ApplicationRequired' }
    @{ LogicalName='cgmp_location';              DisplayName='Location';                      MaxLength=200;  Required='None' }
    @{ LogicalName='cgmp_region';                DisplayName='Region';                        MaxLength=100;  Required='None' }
    @{ LogicalName='cgmp_country';               DisplayName='Country';                       MaxLength=100;  Required='None' }
    @{ LogicalName='cgmp_primaryconnectivity';   DisplayName='Primary Connectivity Type';     MaxLength=300;  Required='None' }
    @{ LogicalName='cgmp_secondaryconnectivity'; DisplayName='Secondary Connectivity Type';   MaxLength=300;  Required='None' }
    @{ LogicalName='cgmp_primarypop';            DisplayName='Primary POP';                   MaxLength=100;  Required='None' }
    @{ LogicalName='cgmp_secondarypop';          DisplayName='Secondary POP';                 MaxLength=100;  Required='None' }
    @{ LogicalName='cgmp_primarylinkprovider';   DisplayName='Primary Link Provider';         MaxLength=200;  Required='None' }
    @{ LogicalName='cgmp_secondarylinkprovider'; DisplayName='Secondary Link Provider';       MaxLength=200;  Required='None' }
    @{ LogicalName='cgmp_primarytelephony';      DisplayName='Primary Telephony Solution';    MaxLength=200;  Required='None' }
    @{ LogicalName='cgmp_secondarytelephony';    DisplayName='Secondary Telephony Solution';  MaxLength=200;  Required='None' }
    @{ LogicalName='cgmp_techpoc';               DisplayName='Tech POC';                      MaxLength=200;  Required='None' }
)
...
function Get-TokenViaBrowser {
    ...
    # ── PKCE: verifier + SHA-256 challenge ─────────────────────────────
    ...
    # ── Start local listener on localhost:Port ──────────────────────────
    ...
    # ── Wait for redirect (async with timeout) ────────────────────────
    $task = $listener.GetContextAsync()
    if (-not $task.Wait(180000)) {   # 3 minutes
        $listener.Stop()
...
"@
    ...
    foreach ($pair in $rawQuery.TrimStart('?').Split('&', [StringSplitOptions]::RemoveEmptyEntries)) {
        $kv = $pair.Split('=', 2)
        if ($kv.Count -eq 2) {
            $qs[$kv[0]] = [uri]::UnescapeDataString($kv[1].Replace('+', ' '))
        }
    }

    if ($qs.ContainsKey('error')) {
        Write-Fail "Sign-in error: $($qs['error']) — $($qs['error_description'])"
    }
    if (-not $qs.ContainsKey('code')) {
        Write-Fail "No authorization code in redirect. Check the browser window for error messages."
    }
...
function Get-AccessToken {
    ...
    # 1. Az PowerShell module — instant if the user is already signed in
    if (Get-Command Get-AzAccessToken -ErrorAction SilentlyContinue) {
        try {
            ...
            Write-Warn "Az module not logged in: $($_.Exception.Message)"
        }
    }

    # 2. Azure CLI — instant if the user is already signed in
    if (Get-Command az -ErrorAction SilentlyContinue) {
        try {
            ...
            Write-Warn "Azure CLI not logged in: $($_.Exception.Message)"
        }
    }

    # 3. Browser sign-in (Authorization Code + PKCE) — no device code
...
function Assert-EntityExists([hashtable]$h) {
    try {
        ...
        if ($status -eq 404) {
            Write-Fail ("Entity '$Entity' not found in this org. " +
                        "Check the logical name in Maker Portal → Solutions → $Solution → Tables.")
        }
        throw
    }
}
...
function Test-ColumnExists([hashtable]$h, [string]$logicalName) {
    try {
        $url = "$ApiBase/EntityDefinitions(LogicalName='$Entity')/Attributes" +
               "(LogicalName='$logicalName')?" + '$select=LogicalName'
...
function New-StringColumn([hashtable]$h, [hashtable]$col) {
    ...
    # POST creates the attribute (returns 204 No Content)
    ...
    # GET it back to retrieve the MetadataId needed for AddSolutionComponent
    $getUrl = "$ApiBase/EntityDefinitions(LogicalName='$Entity')" +
              "/Attributes(LogicalName='$($col.LogicalName)')?" + '$select=MetadataId'
...
function Add-ToSolution([hashtable]$h, [string]$metadataId) {
    ...
    Invoke-RestMethod -Method Post `
        -Uri "$ApiBase/AddSolutionComponent" -Headers $h -Body $body | Out-Null
}
...
function Publish-Entity([hashtable]$h) {
    ...
    Invoke-RestMethod -Method Post `
        -Uri "$ApiBase/PublishXml" -Headers $h -Body $body | Out-Null
}

# ── Main ──────────────────────────────────────────────────────────────────
Write-Host ""
Write-Host "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" -ForegroundColor DarkGray
Write-Host "  CGMP — Add Schema Columns to $Entity" -ForegroundColor White
Write-Host "  Tenant:   $TenantId"
Write-Host "  Org:      $OrgUrl  ($OrgId)"
...
Write-Step "Checking / creating $($Columns.Count) columns on $Entity"
...
foreach ($col in $Columns) {
    if (Test-ColumnExists $headers $col.LogicalName) {
        ...
        Write-Host "   + $($col.DisplayName) ($($col.LogicalName))" `
...
if ($created -gt 0 -and -not $SkipExport) {
    ...
    Write-Host "    2.  npm run regen" -ForegroundColor Gray
