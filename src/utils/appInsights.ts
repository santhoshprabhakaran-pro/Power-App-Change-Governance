...
export function initAppInsights(connectionString?: string): void {
  const cs = connectionString ?? (
    ...
    (() => { try { return localStorage.getItem(INSTRUMENTATION_KEY_LS) ?? ''; } catch { return ''; } })()
  );
  if (!cs || ai) return;
...
export function configureAppInsights(connectionString: string): void {
  if (import.meta.env.DEV && import.meta.env.VITE_APPINSIGHTS_CS) {
    console.warn('[AppInsights] VITE_APPINSIGHTS_CS env var is already set; calling configureAppInsights() will override it for this session only.');
  }
  try { localStorage.setItem(INSTRUMENTATION_KEY_LS, connectionString); } catch {}
...
export function setUser(upn: string): void {
  ...
  const pseudoId = btoa(upn).slice(0, 8).replace(/[^a-zA-Z0-9]/g, '_');
