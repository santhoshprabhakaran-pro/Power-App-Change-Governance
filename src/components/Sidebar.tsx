...
const ICONS: Record<string, string> = {
  ...
  archive:
    'M20.54 5.23l-1.39-1.68C18.88 3.21 18.47 3 18 3H6c-.47 0-.88.21-1.16.55L3.46 5.23C3.17 5.57 3 6.02 3 6.5V19c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V6.5c0-.48-.17-.93-.46-1.27zM6.24 5h11.52l.83 1H5.42l.82-1zM5 19V8h14v11H5z',
  notification:
  ...
  'admin-dash':
    'M17 12h-5v5h5v-5zM16 1v2H8V1H6v2H5c-1.11 0-1.99.9-1.99 2L3 19c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm3 18H5V8h14v11z',
  tasks:
  ...
  gantt: 'M3 5h2V3H3v2zm0 8h2v-2H3v2zm4-8h14V3H7v2zm0 8h14v-2H7v2zm-4 8h2v-2H3v2zm4 0h14v-2H7v2z',
};
...
const ALL_NAV_ITEMS: NavItemDef[] = [
  { id: 'dashboard', label: 'Dashboard', icon: <Icon path={ICONS.dashboard} /> },
  {
    id: 'pmo',
    label: 'PMO Workspace',
    icon: <Icon path={ICONS.pmo} />,
    section: 'WORKSPACES',
    roles: [100000000, 100000001],
  },
  ...
  { id: 'knowledge', label: 'Knowledge Base', icon: <Icon path={ICONS.knowledge} /> },
  {
    ...
    roles: [100000000, 100000001, 100000003],
  },
  {
    id: 'powerbi',
    label: 'Power BI Analytics',
    icon: <Icon path={ICONS.powerbi} />,
    roles: [100000000, 100000001, 100000003],
  },
  ...
  { id: 'notification-rules', label: 'Notification Rules', icon: <Icon path={ICONS.rules} /> },
  {
    ...
    icon: <Icon path={ICONS.templates} />,
    section: 'ADMIN',
    roles: [100000000],
  },
  ...
  { id: 'gantt', label: 'Timeline', icon: <Icon path={ICONS.gantt} /> },
  { id: 'scheduling', label: 'Scheduling Calendar', icon: <Icon path={ICONS.scheduling} /> },
  {
    id: 'capacity-planning',
    label: 'Capacity Planning',
    icon: <Icon path={ICONS.capacity} />,
    roles: [100000000, 100000001, 100000003],
  },
];
...
export default function Sidebar({
  ...
  const { userLoading, assignedLocations } = useApp();
  const { isITOps, isObserver, isDeptAdmin, role } = usePermissions();

  const navItems = useMemo(() => {
    if (userLoading || role === undefined) return ALL_NAV_ITEMS.filter((item) => !item.roles);
    // Observer sees all items (read-only enforced at component level)
    if (isObserver) return ALL_NAV_ITEMS;
    // ISMDeputy gets same access as ISM
    if (role === EXTENDED_ROLES.ISMDeputy) {
      return ALL_NAV_ITEMS.filter(
        (item) => !item.roles || item.roles.includes(100000003) || item.roles.includes(100000000)
      );
    }
    // DeptAdmin gets same access as Admin
    if (isDeptAdmin) return ALL_NAV_ITEMS;
    return ALL_NAV_ITEMS.filter((item) => !item.roles || item.roles.includes(role));
  }, [role, userLoading, isObserver, isDeptAdmin]);
      ...
      <nav className="sidebar__nav" aria-label="Main navigation">
        {navItems.map((item) => (
          <React.Fragment key={item.id}>
            {item.section && !collapsed && <div className="sidebar__section-label">{item.section}</div>}
