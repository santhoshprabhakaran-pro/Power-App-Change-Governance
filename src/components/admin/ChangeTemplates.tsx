...
const EMPTY_FORM = {
  name: '', category: '', changetype: '', risklevel: '', impactlevel: '',
  location: '', region: '', country: '', description: '', projectids: '',
  timeline: '', uatrequired: false, isemergency: false,
};

function TemplateCard({ t, onEdit, onDelete, isDeleting }: { t: Cgmp_changetemplates; onEdit: () => void; onDelete: () => void; isDeleting?: boolean }) {
    ...
    <div className="settings-card" style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 6 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: 'var(--text-secondary)', marginTop: 2 }}>{cat} · {risk} Risk</div>
        </div>
        <div style={{ display: 'flex', gap: 4 }}>
          <button className="btn btn--xs btn--outline" onClick={onEdit}>Edit</button>
          <button className="btn btn--xs btn--danger-outline" onClick={onDelete} disabled={isDeleting}>Delete</button>
        </div>
      </div>
      {t.cgmp_description && <p style={{ fontSize: 12, color: 'var(--text-secondary)', margin: 0 }}>{t.cgmp_description.slice(0, 120)}{(t.cgmp_description.length ?? 0) > 120 ? '…' : ''}</p>}
...
export default function ChangeTemplates() {
  ...
  const handleSave = async () => {
    if (!form.name.trim()) { showToast('error', 'Template name is required'); return; }
    setSaving(true);
    try {
      const payload = {
    ...
    <div className="module-workspace">
      ...
      {!loading && templates.length === 0 && (
        <div className="module-empty">
          No templates yet. {isAdmin ? 'Create your first template using the button above.' : 'Contact your admin to create templates.'}
        </div>
      )}

      {!loading && templates.length > 0 && (
        <div style={{ padding: '12px 24px', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 12 }}>
          {templates.map(t => (
            <TemplateCard
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div>
            ...
            <input className="ff-input" value={form.name} onChange={e => set('name', e.target.value)} placeholder="e.g. Standard Software Deployment" />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            <div>
              <label className="ff-label">Category</label>
              <select className="ff-input ff-select" value={form.category} onChange={e => set('category', e.target.value)}>
            ...
            <div>
              ...
              <select className="ff-input ff-select" value={form.changetype} onChange={e => set('changetype', e.target.value)}>
                <option value="">Select…</option>
            ...
            <div>
              ...
              <select className="ff-input ff-select" value={form.risklevel} onChange={e => set('risklevel', e.target.value)}>
                <option value="">Select…</option>
            ...
            <div>
              ...
              <select className="ff-input ff-select" value={form.impactlevel} onChange={e => set('impactlevel', e.target.value)}>
                <option value="">Select…</option>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
            <div>
              ...
              <input className="ff-input" value={form.location} onChange={e => set('location', e.target.value)} placeholder="e.g. Chennai" />
            </div>
            <div>
              <label className="ff-label">Region</label>
              <input className="ff-input" value={form.region} onChange={e => set('region', e.target.value)} placeholder="e.g. APAC" />
            </div>
            <div>
              <label className="ff-label">Country</label>
              <input className="ff-input" value={form.country} onChange={e => set('country', e.target.value)} placeholder="e.g. India" />
            </div>
          </div>
          <div>
            <label className="ff-label">Default Description</label>
            <textarea className="ff-input" rows={3} value={form.description} onChange={e => set('description', e.target.value)} placeholder="Default change description…" style={{ resize: 'vertical', fontFamily: 'inherit' }} />
          </div>
          <div>
            <label className="ff-label">Default Timeline</label>
            <input className="ff-input" value={form.timeline} onChange={e => set('timeline', e.target.value)} placeholder="e.g. 2 hours maintenance window" />
          </div>
          <div style={{ display: 'flex', gap: 24 }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 13 }}>
              <input type="checkbox" checked={form.uatrequired} onChange={e => set('uatrequired', e.target.checked)} />
              UAT Required
            </label>
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 13 }}>
