...
function highlightCode(code: string, lang: string): string {
  ...
  let work = code;
  // Comments
  work = work.replace(CODE_KW.comment, (m) => {
    tokens.push({ type: 'comment', val: m });
    return PLACEHOLDER + (tokens.length - 1) + PLACEHOLDER;
  });
  // Strings
  CODE_KW.str.lastIndex = 0;
  work = work.replace(CODE_KW.str, (m) => {
    tokens.push({ type: 'str', val: m });
    return PLACEHOLDER + (tokens.length - 1) + PLACEHOLDER;
  });
  // Numbers
  ...
  work = work.replace(new RegExp(`${PLACEHOLDER}(\\d+)${PLACEHOLDER}`, 'g'), (_, idx) => {
    ...
    return t.type === 'comment' ? `<span class="ch-comment">${t.val}</span>` : `<span class="ch-str">${t.val}</span>`;
  });
  return work;
}
...
function renderMarkdown(md: string): string {
  ...
  const mdStripped = md.replace(/```(\w*)\r?\n?([\s\S]*?)```/g, (_, lang: string, code: string) => {
    ...
    codeBlocks.push(
      `<pre class="kb-code-block"${cls}>${safeLang ? `<span class="kb-code-lang">${safeLang}</span>` : ''}<code>${highlighted}</code></pre>`
    );
  ...
  let html = mdStripped
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;') // HTML-encode remaining content
  ...
  codeBlocks.forEach((block, i) => {
    html = html.replace(`CGMPCODEBLOCK${i}END`, block);
  });
  return html;
}
...
export default function KnowledgeBase() {
  ...
  // Related articles: 3 published articles from the same category as the currently reading article
  const relatedArticles = useMemo(() => {
    if (!readingArticle) return [];
    const myCat = readingArticle.cre09_cgmp_category as unknown as number;
    return articles
      .filter(
        (a) =>
          a.cre09_knowledgebasearticleid !== readingArticle.cre09_knowledgebasearticleid &&
          a.cre09_cgmp_ispublished &&
          (a.cre09_cgmp_category as unknown as number) === myCat
      )
      .slice(0, 3);
  ...
  const reviewQueue = useMemo(() => {
    return articles
      ...
      .map((a) => {
        ...
        const daysOverdue = dateStr ? Math.floor((Date.now() - new Date(dateStr).getTime()) / 86400000) : 0;
  ...
  const openRead = useCallback((article: Cre09_knowledgebasearticles) => {
    ...
    if (lastViewedRef.current !== article.cre09_knowledgebasearticleid) {
      ...
      Cre09_knowledgebasearticlesService.update(article.cre09_knowledgebasearticleid, {
        ...
        if (import.meta.env.DEV) console.error('View count update failed:', err);
      });
    }
  }, []);
  ...
  const openCreate = () => {
    if (!canManageKB) {
      showToast('error', 'Only Admins and PMO members can create articles');
      return;
    }
    setEditArticle(null);
    setForm(EMPTY_FORM);
  ...
  const openEdit = (a: Cre09_knowledgebasearticles) => {
    if (!canManageKB) {
      showToast('error', 'Only Admins and PMO members can edit articles');
      return;
    }
    setEditArticle(a);
    setForm({
  ...
  const setField =
    (k: keyof ArticleForm) => (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) =>
      setForm((prev) => ({
        ...prev,
  ...
  const handleSave = async () => {
    if (!canManageKB) {
      showToast('error', 'Only Admins and PMO members can save articles');
      return;
    }
    if (!form.title.trim()) {
      ...
      if (editArticle) {
        ...
        AnnotationsService.create({
          ...
          notetext: JSON.stringify({
            title: payload.cre09_cgmp_title,
            author: payload.cre09_cgmp_authorname ?? '',
  ...
  const handleDelete = async () => {
    ...
    if (!canManageKB) {
      showToast('error', 'Only Admins and PMO members can delete articles');
      return;
    }
    try {
  ...
  const doBulkPublish = async (publish: boolean) => {
    ...
    try {
      await Promise.all(
        ids.map((id) => Cre09_knowledgebasearticlesService.update(id, { cre09_cgmp_ispublished: publish } as any))
      );
      showToast(
        'success',
        `${ids.length} article${ids.length !== 1 ? 's' : ''} ${publish ? 'published' : 'unpublished'}`
      );
  ...
  return (
    <div className="module-workspace">
      ...
      <div className="ism-tabs" style={{ padding: '0 24px' }}>
        <button
          className={`ism-tab ${kbTab === 'articles' ? 'ism-tab--active' : ''}`}
          onClick={() => setKbTab('articles')}
        >
          All Articles ({articles.length})
        </button>
        <button
          className={`ism-tab ${kbTab === 'review-queue' ? 'ism-tab--active' : ''}`}
          onClick={() => setKbTab('review-queue')}
        >
              ...
              <tbody>
                {reviewQueue.map(({ article: a, daysOverdue }) => (
                  <tr key={a.cre09_knowledgebasearticleid}>
                    ...
                    <td>
                      <span
                        className={`badge badge--status ${CAT_CLASS[a.cre09_cgmp_category as unknown as number] ?? 'status-draft'}`}
                      >
                    ...
                    <td>
                      {canManageKB && (
                        <button className="btn btn--xs btn--outline" onClick={() => openEdit(a)}>
                          Edit &amp; Review
                        </button>
                      )}
                    </td>
      ...
      {kbTab === 'articles' && selectedArticleIds.size > 0 && (
        <div className="kb-bulk-bar">
          <span className="kb-bulk-bar__count">
          ...
          <button className="btn btn--sm btn--outline" onClick={() => setSelectedArticleIds(new Set())}>
            Clear
          </button>
        </div>
      )}
              ...
              <div className="kb-article-grid">
                {filtered.map((a) => (
                  <ArticleCard
                    ...
                    isAdmin={isAdmin}
                    selected={selectedArticleIds.has(a.cre09_knowledgebasearticleid)}
                    onSelectToggle={(id) =>
                ...
                .map(([cat, articles]) => (
                  <div key={cat} className="kb-category-group">
                    <div className="kb-category-header">
                      <span className={`badge badge--status ${CAT_CLASS[cat] ?? 'status-draft'}`}>
                        {CAT_LABEL[cat] ?? 'Unknown'}
                      </span>
      ...
      <SlidePanel
        ...
        subtitle={
          readingArticle
            ? `${CAT_LABEL[readingArticle.cre09_cgmp_category as unknown as number] ?? ''} · By ${readingArticle.cre09_cgmp_authorname ?? 'Unknown'} · ${readingArticle.cre09_cgmp_viewcount ?? 0} views`
            : ''
        }
        width={640}
      >
          ...
          <div className="kb-reader">
            ...
            {isStale(readingArticle.cre09_cgmp_tags) && (
              <div className="kb-stale-banner" role="alert">
                ⚠ This article is past its review date ({extractReviewDate(readingArticle.cre09_cgmp_tags)}). Please
            ...
            <div
              ...
              dangerouslySetInnerHTML={{
                __html: sanitizeHtml(renderMarkdown(readingArticle.cre09_cgmp_content ?? '')),
              }}
            />

            <div className="kb-article-footer">
              {canManageKB && (
                <button className="btn btn--xs btn--outline" onClick={() => openEdit(readingArticle)}>
                  Edit Article
                </button>
              )}
                ...
                <div style={{ marginTop: 16 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {[...edits].reverse().map((e, i) => {
                      ...
                      try {
                        info = JSON.parse(e.notetext ?? '{}');
                      } catch {}
                      return (
                        <div
                          key={e.annotationid ?? i}
                          style={{
                            display: 'flex',
                            gap: 8,
                            fontSize: 12,
                        ...
                        >
                          <span style={{ color: 'var(--text-tertiary)', minWidth: 120 }}>
                            {fmtDateTimeShort(info.timestamp)}
                          </span>
              ...
              {annLoading ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 12 }}>
                  {kbAnnotations.filter((a) => a.subject === 'KB Comment').length === 0 ? (
                    ...
                    kbAnnotations
                      .filter((a) => a.subject === 'KB Comment')
                      .map((c, i) => (
                        <div
                          key={c.annotationid ?? i}
                          style={{
                            padding: '8px 12px',
                            background: 'var(--surface)',
                            border: '1px solid var(--border)',
              ...
              <div className="kb-related">
                ...
                <div className="kb-related__list">
                  {relatedArticles.map((a) => (
                    ...
                    >
                      <span
                        className={`badge badge--status ${CAT_CLASS[a.cre09_cgmp_category as unknown as number] ?? ''}`}
                        style={{ fontSize: 10 }}
                      >
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14, paddingTop: 4 }}>
          <div className="ff-row">
            <div className="ff-group" style={{ flex: 2 }}>
              <label className="ff-label">
                Title <span className="ff-required">*</span>
              </label>
          ...
          <div className="ff-group">
            ...
            />
            <span style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>
              If set, shows a stale warning after this date
            </span>
          </div>
          ...
          <div className="ff-group">
            ...
            <input
              ...
              onChange={setField('relatedchangeid')}
              placeholder="CHG-XXXXXX"
            />
            <span style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>
              Link this article to a specific change by its change number
            </span>
          </div>

          <div className="ff-group">
            <label className="ff-label">
              Content (Markdown) <span className="ff-required">*</span>
            </label>
            ...
            {!preview && (
              <div className="kb-toolbar">
                <button
                  ...
                  className="kb-tb-btn"
                  title="Bold"
                  onClick={() => insertMarkdown(textareaRef, '**', '**')}
                >
                  B
                </button>
                <button
                  type="button"
                  className="kb-tb-btn kb-tb-btn--italic"
                  title="Italic"
                  onClick={() => insertMarkdown(textareaRef, '*', '*')}
                >
                  I
                ...
                <button
                  ...
                  className="kb-tb-btn"
                  title="Numbered List"
                  onClick={() => insertMarkdown(textareaRef, '1. ', '')}
                >
                  1.
                </button>
                <button
                ...
                <button
                  ...
                  className="kb-tb-btn"
                  title="Link"
                  onClick={() => insertMarkdown(textareaRef, '[', '](https://)', 'link text')}
                >
                  🔗
                </button>
              </div>
  ...
  return (
    <div className={`kb-card${selected ? ' kb-card--selected' : ''}`}>
      <div className="kb-card__header">
        ...
        <span className={`badge badge--status ${CAT_CLASS[cat] ?? 'status-draft'}`} style={{ fontSize: 10 }}>
          {CAT_LABEL[cat]}
        </span>
        {!article.cre09_cgmp_ispublished && <span className="kb-draft-badge">Draft</span>}
        {/* Feature 9.4: Stale badge in card header */}
        {isStale(article.cre09_cgmp_tags) && (
          <span className="kb-stale-badge" title="Review date has passed">
            ⚠ Stale
          </span>
        )}
      </div>
      <button className="kb-card__title" onClick={() => onRead(article)}>
        {article.cre09_cgmp_title || 'Untitled'}
      </button>
      <div className="kb-card__meta">
        {article.cre09_cgmp_authorname && <span>{article.cre09_cgmp_authorname}</span>}
        {article.modifiedon && <span title="Last updated">{fmtDateTimeShort(article.modifiedon)}</span>}
      ...
      <div className="kb-card__actions">
        {canManageKB && (
          <button className="btn btn--xs btn--outline" onClick={() => onEdit(article)}>
            Edit
          </button>
        )}
        {isAdmin && (
          <button className="btn btn--xs btn--danger-outline" onClick={() => onDelete(article)}>
            Delete
          </button>
        )}
      </div>
