...
import type { Cgmp_notifications, Cgmp_notificationscgmp_priority } from '../../generated/models/Cgmp_notificationsModel';
import { useApp } from '../../context/AppContext';
import { useNotifications } from '../../hooks/useDataverse';
import ConfirmDialog from '../ui/ConfirmDialog';

import { PRIORITY_CONFIG, CAT_LABEL, CAT_COLOR, loadPinned, savePinned, timeAgo, fullDate, getGroup, GROUP_LABEL, GROUP_ORDER } from '../../utils/notifications';
...
function renderWithLinks(text: string | undefined, navigate: (page: string) => void): React.ReactNode {
  ...
  while ((m = re.exec(text)) !== null) {
    ...
    const chg = m[1];
    parts.push(
      <button key={m.index} className="notif-chg-link" onClick={e => {
        e.stopPropagation();
        try { localStorage.setItem(PMO_LS_KEY, JSON.stringify({ search: chg, status: '', risk: '', category: '', dateRange: 'all' })); } catch {}
        navigate('pmo');
      }}>{chg}</button>
    );
...
interface ExportItem { id: string; title: string; message: string; priority: string; category: string; created: string; read: boolean; dismissed: boolean; }

function exportJSON(items: ExportItem[]) {
  const blob = new Blob([JSON.stringify(items, null, 2)], { type: 'application/json' });
  const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'notifications.json'; a.click();
}
...
function NotifCard({ n, pinned, onPin, onMarkRead, onSnooze, onDismiss, onDelete, onEscalate, working, navigate }: CardProps) {
  ...
  useEffect(() => {
    ...
    const h = (e: MouseEvent) => { if (snoozeRef.current && !snoozeRef.current.contains(e.target as Node)) setSnoozeOpen(false); };
      ...
      <div className="nc2-card__content">
        <div className="nc2-card__header">
          <div className="nc2-card__chips">
            <span className="nc2-chip" style={{ background: CAT_COLOR[cCode] + '18', color: CAT_COLOR[cCode] }}>{CAT_LABEL[cCode] ?? 'System'}</span>
            <span className="nc2-chip" style={{ background: pri.badgeBg, color: pri.badgeColor }}>{pri.label}</span>
          ...
          <span className="nc2-card__time" title={fullDate(n.createdon)}>{timeAgo(n.createdon)}</span>
        </div>
        ...
        {n.cgmp_message && n.cgmp_message.length > 120 && (
          <button className="nc2-expand-btn" onClick={() => setExpanded(e => !e)}>{expanded ? 'Show less' : 'Show more'}</button>
        )}
        {n.cgmp_actionurl && (
          <a
        ...
        <div className="nc2-card__actions">
          {!n.cgmp_isread && (
            <button className="nc2-action" onClick={() => onMarkRead(n.cgmp_notificationid)} disabled={isWorking}>Mark Read</button>
          )}
          <button className={`nc2-action${isPinned ? ' nc2-action--active' : ''}`} onClick={() => onPin(n.cgmp_notificationid)}>
            {isPinned ? 'Unpin' : 'Pin'}
          </button>
          <div className="nc2-snooze-wrap" ref={snoozeRef}>
            <button className="nc2-action" onClick={() => setSnoozeOpen(o => !o)} disabled={isWorking}>Snooze</button>
            {snoozeOpen && (
              <div className="nc2-snooze-menu">
                {[1, 4, 24].map(h => (
                  <button key={h} className="nc2-snooze-opt" onClick={() => { onSnooze(n.cgmp_notificationid, h); setSnoozeOpen(false); }}>
          ...
          {pCode === 100000002 && (
            <button className="nc2-action" onClick={() => onEscalate(n.cgmp_notificationid)} disabled={isWorking}>Escalate</button>
          )}
          {!n.cgmp_isdismissed && (
            <button className="nc2-action nc2-action--danger" onClick={() => onDismiss(n.cgmp_notificationid)} disabled={isWorking}>Dismiss</button>
          )}
          {n.cgmp_isdismissed && (
            <button className="nc2-action nc2-action--danger" onClick={() => onDelete(n.cgmp_notificationid)} disabled={isWorking} title="Permanently delete this notification">Delete</button>
          )}
        </div>
      </div>
    </div>
...
export default function NotificationCenter() {
  ...
  const { notifications, loading, refresh } = useNotifications(userProfile?.cgmp_userprofileid);
  ...
  const handleMarkRead = useCallback(async (id: string) => {
    setW(id, true);
    try { await Cgmp_notificationsService.update(id, { cgmp_isread: true }); refresh(); } catch { showToast('error', 'Failed to mark notification as read'); } finally { setW(id, false); }
  }, [refresh, showToast]);
  ...
  const handleSnooze = useCallback(async (id: string, hours: number) => {
    ...
    try {
      ...
      refresh();
    } catch { showToast('error', 'Failed to snooze notification'); } finally { setW(id, false); }
  }, [refresh, showToast]);

  const handleDismiss = useCallback(async (id: string) => {
    setW(id, true);
    try { await Cgmp_notificationsService.update(id, { cgmp_isdismissed: true }); refresh(); } catch { showToast('error', 'Failed to dismiss notification'); } finally { setW(id, false); }
  }, [refresh, showToast]);

  const handleEscalate = useCallback(async (id: string) => {
    setW(id, true);
    try { await Cgmp_notificationsService.update(id, { cgmp_priority: 100000000 as unknown as Cgmp_notificationscgmp_priority }); refresh(); } catch { showToast('error', 'Failed to escalate notification'); } finally { setW(id, false); }
  }, [refresh, showToast]);
  ...
  const handleMarkAllRead = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < toMark.length; i += 20) {
        await Promise.all(toMark.slice(i, i + 20).map(n =>
          Cgmp_notificationsService.update(n.cgmp_notificationid, { cgmp_isread: true })
        ));
      }
      refresh();
    } catch { showToast('error', 'Failed to mark all notifications as read'); } finally { setBulkWorking(false); }
  }, [notifications, refresh, showToast]);

  const handleDismissAll = useCallback(async (toProcess: Cgmp_notifications[]) => {
    setBulkWorking(true);
    try {
      await Promise.all(toProcess.map(n => Cgmp_notificationsService.update(n.cgmp_notificationid, { cgmp_isdismissed: true })));
      refresh();
    } catch { showToast('error', 'Failed to dismiss notifications'); } finally { setBulkWorking(false); }
  }, [refresh, showToast]);

  const handleDelete = useCallback(async (id: string) => {
    setW(id, true);
    try { await Cgmp_notificationsService.delete(id); refresh(); } catch { showToast('error', 'Failed to delete notification'); } finally { setW(id, false); }
  }, [refresh, showToast]);
  ...
  const handleDeleteAllDismissed = useCallback(async () => {
    ...
    try {
      ...
      refresh();
    } catch { showToast('error', 'Failed to delete dismissed notifications'); } finally { setBulkWorking(false); }
  }, [notifications, refresh, showToast]);

  const filtered = useMemo(() => {
    let list = [...notifications];
    if (tab === 'unread') list = list.filter(n => !n.cgmp_isread && !n.cgmp_isdismissed);
    else if (tab === 'high') list = list.filter(n => (n.cgmp_priority as unknown as number) === 100000000 && !n.cgmp_isdismissed);
    else if (tab === 'pinned') list = list.filter(n => pinned.has(n.cgmp_notificationid));
    else if (tab === 'snoozed') list = list.filter(n => n.cgmp_snoozeduntil && new Date(n.cgmp_snoozeduntil) > new Date());
    ...
    return list;
  }, [notifications, tab, catFilter, priFilter, dateFrom, dateTo, search, pinned]);
    ...
    <div className="nc2-root">
      ...
      <div className="nc2-header">
        ...
        <div className="nc2-header__actions">
          <button className="btn btn--outline btn--sm" onClick={() => exportCSV('notifications.csv', ['ID', 'Title', 'Message', 'Priority', 'Category', 'Created', 'Read', 'Dismissed'], exportItems.map(i => [i.id, i.title, i.message, i.priority, i.category, i.created, String(i.read), String(i.dismissed)]))}>Export CSV</button>
          <button className="btn btn--outline btn--sm" onClick={() => exportJSON(exportItems)}>Export JSON</button>
      ...
      <div className="nc2-toolbar">
        ...
        <div className="nc2-toolbar__right">
          <div className="nc2-search-wrap">
            <svg className="nc2-search-icon" width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M15.5 14h-.79l-.28-.27C15.41 12.59 16 11.11 16 9.5 16 5.91 13.09 3 9.5 3S3 5.91 3 9.5 5.91 16 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"/></svg>
            <input className="nc2-search" type="search" placeholder="Search notifications…" value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} />
          </div>
          <button className={`btn btn--outline btn--sm${showFilters ? ' btn--active' : ''}`} onClick={() => setShowFilters(f => !f)}>
      ...
      {showFilters && (
        <div className="nc2-filter-bar">
          <select className="form-select form-select--sm" value={catFilter} onChange={e => { setCatFilter(e.target.value); setPage(1); }}>
            {CAT_OPTS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
          </select>
          <select className="form-select form-select--sm" value={priFilter} onChange={e => { setPriFilter(e.target.value); setPage(1); }}>
            {PRI_OPTS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
          </select>
          <input className="form-input form-input--sm" type="date" value={dateFrom} onChange={e => { setDateFrom(e.target.value); setPage(1); }} title="From date" />
          <input className="form-input form-input--sm" type="date" value={dateTo} onChange={e => { setDateTo(e.target.value); setPage(1); }} title="To date" />
          {(catFilter || priFilter || dateFrom || dateTo) && (
            <button className="btn btn--ghost btn--sm" onClick={() => { setCatFilter(''); setPriFilter(''); setDateFrom(''); setDateTo(''); }}>Clear</button>
          )}
        </div>
      )}
      ...
      {stats.unread > 0 && tab !== 'dismissed' && (
        <div className="nc2-bulk-bar">
          <button className="btn btn--outline btn--sm" onClick={handleMarkAllRead} disabled={bulkWorking}>Mark All Read</button>
          <button className="btn btn--outline btn--sm" onClick={() => setDismissConfirmItems(filtered.filter(n => !n.cgmp_isdismissed))} disabled={bulkWorking}>Dismiss All</button>
          <span className="nc2-bulk-info">{filtered.length} notification{filtered.length !== 1 ? 's' : ''} visible</span>
        </div>
      )}
      ...
      <div className="nc2-list">
        {loading ? (
          ...
          <div className="nc2-empty">
            <svg width="48" height="48" viewBox="0 0 24 24" fill="currentColor"><path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.9 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z"/></svg>
          ...
          <>
            {pinnedItems.length > 0 && (
              <section>
                ...
                {pinnedItems.map(n => (
                  <NotifCard key={n.cgmp_notificationid} n={n} pinned={pinned} onPin={handlePin} onMarkRead={handleMarkRead} onSnooze={handleSnooze} onDismiss={handleDismiss} onDelete={handleDelete} onEscalate={handleEscalate} working={working} navigate={navigate} />
                ))}
              </section>
            )}
      ...
      <ConfirmDialog
        ...
        onConfirm={() => { handleDismissAll(dismissConfirmItems); setDismissConfirmItems([]); }}
      ...
      <ConfirmDialog
        ...
        onConfirm={() => { setDeleteAllConfirm(false); handleDeleteAllDismissed(); }}
