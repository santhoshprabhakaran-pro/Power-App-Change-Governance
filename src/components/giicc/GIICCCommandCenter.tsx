...
export function parseChangeUATData(json: string | undefined): ChangeUATData {
  if (!json?.trim()) return {};
  try {
    return JSON.parse(json) as ChangeUATData;
  } catch {
    return {};
  }
}
  ...
  return (
    <>
      <span
        ...
        onClick={(e) => {
          e.stopPropagation();
          open ? setOpen(false) : openTooltip();
        }}
...
export function RemediationHistory({ history }: { history: RemediationHistoryEntry[] }) {
  if (!history || history.length === 0)
    return <span style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>No history yet</span>;
  return (
          ...
          <div className="rem-history__body">
            <div className="rem-history__row">
              <span
                ...
                style={{ background: `${remStatusColor(h.status)}20`, color: remStatusColor(h.status) }}
              >
                {remStatusLabel(h.status)}
              </span>
              {h.previousStatus && (
                <span className="rem-history__from">
                  from {remStatusLabel(h.previousStatus as FailedProjectStatus)}
                </span>
              )}
            </div>
...
function uatStatusClass(s: UATStatus): string {
  const m: Record<UATStatus, string> = {
    Pending: 'uat-s--pending',
    Success: 'uat-s--success',
    Failed: 'uat-s--failed',
    Rollback: 'uat-s--rollback',
    NotApplicable: 'uat-s--na',
    PartialSuccess: 'uat-s--partial',
  };
  return m[s] ?? '';
}
...
function bridgeStatusClass(code: number): string {
  return (
    (
      {
        100000000: 'bridge--active',
        100000001: 'bridge--completed',
        100000002: 'bridge--failed',
        100000003: 'bridge--cancelled',
        100000004: 'bridge--scheduled',
...
function bridgeBadgeClass(code: number): string {
  return (
    (
      {
        100000000: 'status-inprogress',
        100000001: 'status-completed',
        100000002: 'status-failed',
        100000003: 'status-cancelled',
        100000004: 'status-published',
      } as Record<number, string>
    )[code] ?? 'status-draft'
  );
}
...
function BridgeCard({
  ...
  bridge: Cgmp_bridges;
  onExecution(b: Cgmp_bridges): void;
  onPIR(b: Cgmp_bridges): void;
  onEdit(b: Cgmp_bridges): void;
  onStart(b: Cgmp_bridges): void;
}) {
  ...
  return (
    <div className={`bridge-card ${bridgeStatusClass(status)}`}>
      <div className="bridge-card__header">
        ...
        {bridge.cgmp_changenumber && (
          <span className="bridge-card__change-ref">Change: {bridge.cgmp_changenumber}</span>
        )}
      </div>
      <div className="bridge-card__meta">
        ...
        {isActive && bridge.cgmp_actualstart && (
          <span className="bridge-live-badge">
            <span className="bridge-live-dot" />
            Live since {fmtTime(bridge.cgmp_actualstart)}
          </span>
        )}
        {isValidTeamsUrl(bridge.cgmp_teamsurl) ? (
          <a
            href={bridge.cgmp_teamsurl}
            target="_blank"
            rel="noopener noreferrer"
            className="bridge-teams-link"
            onClick={(e) => e.stopPropagation()}
          >
            <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor">
              <path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm-7 3.5c1.38 0 2.5 1.12 2.5 2.5S14.38 12.5 13 12.5 10.5 11.38 10.5 10 11.62 7.5 13 7.5zM8 16c0-2.33 3.34-3.5 5-3.5s5 1.17 5 3.5H8z" />
            </svg>
            Join Teams
          </a>
        ) : bridge.cgmp_teamsurl ? (
          <span title="Invalid Teams URL" className="bridge-teams-link bridge-teams-link--invalid">
            {bridge.cgmp_teamsurl}
          </span>
        ) : null}
      </div>
      ...
      <div className="bridge-card__actions">
        <button className="btn btn--xs btn--outline" onClick={() => onEdit(bridge)}>
          Edit
        </button>
        {isScheduled && (
          <button className="btn btn--xs btn--primary" onClick={() => onStart(bridge)}>
            Start Bridge
          </button>
        )}
        {(isActive || isScheduled) && (
          <button className="btn btn--xs btn--secondary" onClick={() => onExecution(bridge)}>
            Execution
          </button>
        )}
        {isActive && (
          <button className="btn btn--xs btn--primary" onClick={() => onPIR(bridge)}>
            PIR
          </button>
        )}
        {(status === BRIDGE_STATUS.Completed || status === BRIDGE_STATUS.Failed) && (
          <button className="btn btn--xs btn--outline" onClick={() => onPIR(bridge)}>
            View PIR
          </button>
        )}
      </div>
...
function ColHeader({ title, count, accent }: { title: string; count: number; accent?: string }) {
  return (
    <div className="giicc-col-header" style={{ borderTopColor: accent }}>
      ...
      <span className="giicc-col-count" style={{ background: accent }}>
        {count}
      </span>
    </div>
  );
...
function exportUATReport(changes: Cgmp_changes[], projects: Cgmp_projects[]) {
  ...
  changes.forEach((c) => {
    ...
    ids.forEach((pid) => {
      ...
      const c1 = e.contacts[0];
      const c2 = e.contacts[1];
      rows.push([
        c.cgmp_changenumber ?? '',
        c.cgmp_title ?? '',
...
function ScheduledChangesPaneInner({
  ...
  const getBulkState = (changeId: string): BulkState =>
    bulkStates[changeId] ?? { field: 'postStatus', status: 'Success' };
    ...
    try {
      ...
      itOpsPocs.forEach((poc) => {
        Cgmp_notificationsService.create({
          ...
          trackAppException(err, { context: 'notification.create' })
        );
      });
  ...
  const doBulkComplete = async () => {
    ...
    if (succeeded < toComplete.length) {
      showToast(
        'warning',
        `${succeeded}/${toComplete.length} changes marked as Completed — ${toComplete.length - succeeded} failed. Check the console for details.`
      );
    } else {
  ...
  const applyBulkUpdate = (changeId: string, originalJson: string | undefined) => {
    ...
    if (!sel?.size) {
      showToast('warning', 'No projects selected');
      return;
    }
    const { field: bulkField, status: bulkStatus } = getBulkState(changeId);
    sel.forEach((pid) =>
      patchEntry(changeId, pid, { [bulkField]: bulkStatus } as Partial<ProjectUATEntry>, originalJson)
    );
  ...
  if (inProgressChanges.length === 0) {
    return (
      <div className="giicc-empty-pane">
        ...
        <span>
          Changes move here when IT Ops clicks <strong>Handover to GIICC</strong>.
        </span>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="giicc-sc-toolbar">
        <div className="giicc-sc-stats">
          <span className="giicc-sc-stat">
            <strong>{inProgressChanges.length}</strong> changes in progress
          </span>
          <span className="giicc-sc-stat">
            <strong>{totalProjects}</strong> projects tracked
          </span>
          <span
            className="giicc-sc-stat"
            style={{ color: totalDone === totalProjects && totalProjects > 0 ? 'var(--success)' : 'inherit' }}
          >
      ...
      {checkedChanges.size > 0 && (
        <div className="giicc-bulk-complete-bar">
          <span>
            {checkedChanges.size} change{checkedChanges.size > 1 ? 's' : ''} selected
          </span>
          ...
          <button className="btn btn--ghost btn--sm" onClick={() => setCheckedChanges(new Set())}>
            Clear
          </button>
        </div>
      )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {inProgressChanges.map((change) => {
          ...
          const projectIds = (change.cgmp_projectids ?? '')
            ...
            .map((s) => s.trim())
            .filter(Boolean);
          const impProjects = projectIds
            .map((pid) => projects.find((p) => p.cgmp_projectid === pid))
            .filter((p): p is Cgmp_projects => !!p);
            ...
            <div key={change.cgmp_changeid} className="giicc-change-block">
              ...
              <div
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter' || e.key === ' ') {
                    e.preventDefault();
                    toggle(change.cgmp_changeid);
                  }
                }}
              >
                <svg
                  className={`giicc-chevron ${isOpen ? 'giicc-chevron--open' : ''}`}
                  width="14"
                  height="14"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                ...
                <div className="giicc-change-info">
                  ...
                  <div className="giicc-change-bottom">
                    ...
                    {change.cgmp_starttime && (
                      <span className="giicc-change-dates">
                        {fmtDate(change.cgmp_starttime)} → {fmtDate(change.cgmp_endtime)}
                      </span>
                    )}
                    <span className="giicc-proj-count-chip">
              ...
              {isOpen && (
                <div className="giicc-change-projects">
                      ...
                      return (
                        <div className="giicc-bulk-bar">
                          ...
                          <select
                            ...
                            value={bs.field}
                            onChange={(e) =>
                              setBulkField(change.cgmp_changeid, e.target.value as 'preStatus' | 'postStatus')
                            }
                          >
                          ...
                          <select
                            className="giicc-sel"
                            value={bs.status}
                            onChange={(e) => setBulkStatus(change.cgmp_changeid, e.target.value as UATStatus)}
                          >
                            {UAT_STATUS_OPTS.map((o) => (
                  ...
                  {impProjects.map((project) => {
                    ...
                    return (
                      <div key={project.cgmp_projectid}>
                        <div className={`giicc-proj-row ${isSelected ? 'giicc-proj-row--selected' : ''}`}>
                          {/* Checkbox */}
                          <div className="giicc-proj-col giicc-proj-col--check">
                            <input
                              type="checkbox"
                            ...
                            <select
                              ...
                              onChange={(e) =>
                                patchEntry(
                                  change.cgmp_changeid,
                                  project.cgmp_projectid,
                                  { preStatus: e.target.value as UATStatus },
                                  change.cgmp_uatusers
                                )
                              }
                            >
                            ...
                            <select
                              ...
                              onChange={(e) =>
                                patchEntry(
                                  change.cgmp_changeid,
                                  project.cgmp_projectid,
                                  { postStatus: e.target.value as UATStatus },
                                  change.cgmp_uatusers
                                )
                              }
                            >
                        ...
                        {isAttachOpen && (
                          ...
                          >
                            <AttachmentPanel objectId={change.cgmp_changeid} subjectPrefix={attachPfx} compact />
                          </div>
                        )}
                      </div>
                    );
                  ...
                  {impProjects.length === 0 && (
                    <div className="giicc-empty" style={{ padding: 16 }}>
                      No impacted projects linked to this change.
                    </div>
                  )}
                </div>
              )}
...
function FailedProjectsPane({
  ...
  const failedChanges = useMemo(() => {
    ...
    return changes.filter((c) => {
      ...
      const uatData = parseChangeUATData(c.cgmp_uatusers);
      return Object.values(uatData).some(
        (e) => e.postStatus === 'Failed' || e.postStatus === 'Rollback' || e.postStatus === 'PartialSuccess'
      );
    });
  }, [changes]);
  ...
  const saveRemediation = useCallback(
    async (change: Cgmp_changes, pid: string) => {
      ...
      try {
        ...
        /* Notifications to ISM + IT Ops (fire-and-forget — never awaited) */
        const proj = projects.find((p) => p.cgmp_projectid === pid);
        const recipients = [
          proj?.cgmp_primaryism,
          proj?.cgmp_backupism,
          change.cgmp_releasedby,
          change.cgmp_reviewedby,
        ...
        showToast(
          'success',
          `Status → ${remStatusLabel(finalStatus)}${draft.status === 'Resolved' ? ' (auto-closed)' : ''}`
        );
      } catch (err) {
    ...
    return (
      <div className="giicc-empty-pane">
        <svg
          ...
          viewBox="0 0 24 24"
          fill="currentColor"
          style={{ opacity: 0.2, color: 'var(--success)' }}
        >
          <path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17z" />
        </svg>
      ...
      {failedChanges.map((change) => {
        ...
        const failedEntries = projectIds
          ...
          .filter(({ entry }) => {
            ...
            if (
              entry.postStatus !== 'Failed' &&
              entry.postStatus !== 'Rollback' &&
              entry.postStatus !== 'PartialSuccess'
            )
              return false;
                ...
                {failedEntries.map(({ pid, project, entry }) => {
                  ...
                  return (
                    <div key={pid} className={`giicc-fpc ${isClosed ? 'giicc-fpc--closed' : ''}`}>
                      {/* Left accent bar */}
                      <div
                        className="giicc-fpc__accent"
                        style={{ background: isClosed ? 'var(--border)' : remStatusColor(currentRemStatus) }}
                      />
                      ...
                      <div className="giicc-fpc__body">
                        ...
                        <div className="giicc-fpc__top">
                          ...
                          >
                            ...
                            {(entry.remediationHistory ?? []).length > 0
                              ? ` (${(entry.remediationHistory ?? []).length})`
                              : ''}
                          </button>
                        </div>
                        ...
                        <div className="giicc-fpc__statuses">
                          <div className="giicc-fpc__stat-group">
                            ...
                            <span className={`giicc-uat-badge ${uatStatusClass(entry.postStatus)}`}>
                              {entry.postStatus}
                            </span>
                          </div>
                          <svg
                          ...
                          <div className="giicc-fpc__stat-group">
                            ...
                            <span
                              className="rem-status-pill giicc-fpc__rem-pill"
                              style={{
                                background: `${remStatusColor(currentRemStatus)}18`,
                                color: remStatusColor(currentRemStatus),
                                borderColor: `${remStatusColor(currentRemStatus)}40`,
                              }}
                            >
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12 }}>
                                  ...
                                  <button
                                    ...
                                    onClick={() => {
                                      ...
                                      setRcaDrafts((prev) => ({ ...prev, [key]: { ...existingRca } }));
                                    }}
                                  >
                                    Edit
                                  </button>
                                ...
                                >
                                  ...
                                  <textarea
                                    ...
                                    onChange={(e) =>
                                      setRcaDrafts((prev) => ({
                                        ...prev,
                                        [key]: { ...prev[key], preventiveAction: e.target.value },
                                      }))
                                    }
                                    style={{ resize: 'vertical', minHeight: 48 }}
                                  />
                                  <input
                                    ...
                                    onChange={(e) =>
                                      setRcaDrafts((prev) => ({
                                        ...prev,
                                        [key]: { ...prev[key], owner: e.target.value },
                                      }))
                                    }
                                  />
...
export default function GIICCCommandCenter() {
  ...
  useEffect(() => {
    ...
    changes.forEach((c) => {
      ...
      Object.entries(d).forEach(([pid, entry]) => {
        ...
        const proj = projects.find((p) => p.cgmp_projectid === pid);
        const candidates = [proj?.cgmp_primaryism, proj?.cgmp_backupism, c.cgmp_releasedby].filter(
          (v): v is string => !!v && v !== currentUserUpn
        );
        const uniqueRecipients = [
          ...new Set(candidates.length ? candidates : ([currentUserUpn].filter(Boolean) as string[])),
        ];
        uniqueRecipients.forEach((recipient) => {
          Cgmp_notificationsService.create({
  ...
  const activeBridges = useMemo(
    () => bridges.filter((b) => (b.cgmp_status as unknown as number) === BRIDGE_STATUS.InProgress),
    [bridges]
  );
  const scheduledBridges = useMemo(
    () => bridges.filter((b) => (b.cgmp_status as unknown as number) === BRIDGE_STATUS.Scheduled),
    [bridges]
  );
  const todayDone = useMemo(
    () =>
      bridges.filter((b) => {
        ...
        return (
          (s === BRIDGE_STATUS.Completed || s === BRIDGE_STATUS.Failed) &&
          (isToday(b.cgmp_actualend) || isToday(b.cgmp_schedend))
        );
      }),
    [bridges]
  );
  ...
  const thisWeekBridges = useMemo(
    () => bridges.filter((b) => b.cgmp_schedstart && new Date(b.cgmp_schedstart).getTime() >= weekStartMs),
    [bridges, weekStartMs]
  );
    ...
    <div className="giicc-workspace">
      ...
      <div className="giicc-kpis">
        ...
        <div className="giicc-kpi">
          <span className="giicc-kpi__value" style={{ color: 'var(--primary)' }}>
            {inProgressCount}
          </span>
        ...
        <div className="giicc-kpi">
          <span
            className="giicc-kpi__value"
            style={{
              color:
                todayDone.filter((b) => (b.cgmp_status as unknown as number) === BRIDGE_STATUS.Completed).length > 0
                  ? 'var(--success)'
                  : undefined,
            }}
          >
        ...
        <div className="giicc-kpi">
          <span
            className="giicc-kpi__value"
            style={{ color: failedCount > 0 ? 'var(--danger)' : 'var(--text-tertiary)' }}
          >
            {failedCount}
          </span>
      ...
      <div className="giicc-tabs">
        <button
          className={`giicc-tab ${tab === 'summary' ? 'giicc-tab--active' : ''}`}
        ...
        <button
          className={`giicc-tab ${tab === 'scheduled' ? 'giicc-tab--active' : ''}`}
        ...
        <button
          className={`giicc-tab ${tab === 'bridges' ? 'giicc-tab--active' : ''}`}
        ...
        >
          ...
          {activeBridges.length > 0 && (
            <span className="giicc-tab-badge giicc-tab-badge--live">{activeBridges.length} live</span>
          )}
        </button>
          ...
          <div className="giicc-summary">
            ...
            <div className="giicc-summary-kpis">
              <div className="giicc-summary-kpi">
                <span
                  className="giicc-summary-kpi__value"
                  style={{ color: activeBridges.length > 0 ? 'var(--danger)' : 'var(--text-tertiary)' }}
                >
              ...
              <div className="giicc-summary-kpi">
                <span
                  className="giicc-summary-kpi__value"
                  style={{ color: pendingHandoverCount > 0 ? 'var(--orange)' : 'var(--text-tertiary)' }}
                >
              ...
              <div className="giicc-summary-kpi">
                <span
                  className="giicc-summary-kpi__value"
                  style={{ color: failedCount > 0 ? 'var(--danger)' : 'var(--success)' }}
                >
                  {failedCount}
                </span>
              ...
              <div className="giicc-summary-kpi">
                <span
                  ...
                  style={{
                    color:
                      thisWeekCompletionRate !== null
                        ? thisWeekCompletionRate >= 80
                          ? 'var(--success)'
                          : thisWeekCompletionRate >= 50
                            ? 'var(--orange)'
                            : 'var(--danger)'
                        : 'var(--text-tertiary)',
                  }}
            ...
            <div className="giicc-summary-panels">
              ...
              <div className="giicc-summary-section">
                ...
                {activeBridges.length === 0 ? (
                  <div className="module-empty" style={{ fontSize: 12 }}>
                    No bridges currently running.
                  </div>
                ) : (
                  activeBridges.map((b) => (
                    <div key={b.cgmp_bridgeid} className="giicc-summary-bridge-row">
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>
                          Started{' '}
                          {b.cgmp_actualstart
                            ? new Date(b.cgmp_actualstart).toLocaleTimeString([], {
                                hour: '2-digit',
                                minute: '2-digit',
                      ...
                      {isValidTeamsUrl(b.cgmp_teamsurl) ? (
                        ...
                        <span
                          title="Invalid Teams URL"
                          style={{ flexShrink: 0, fontSize: 11, color: 'var(--text-secondary)' }}
                        >
                          {b.cgmp_teamsurl}
                        </span>
                      ) : null}
              ...
              <div className="giicc-summary-section">
                ...
                {pendingHandoverCount === 0 ? (
                  <div className="module-empty" style={{ fontSize: 12 }}>
                    No changes pending handover.
                  </div>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {changes
                      .filter((c) => (c.cgmp_status as unknown as number) === STATUS.Locked)
                      .slice(0, 8)
                      .map((c) => {
                        ...
                        return (
                          ...
                          >
                            <span className="change-number" style={{ fontSize: 11, flexShrink: 0 }}>
                              {c.cgmp_changenumber}
                            </span>
                            <span
                              style={{
                            ...
                            <span
                              className={`badge badge--risk ${riskColor(rc)}`}
                    ...
                    {pendingHandoverCount > 8 && (
                      <div style={{ fontSize: 11, color: 'var(--text-tertiary)', textAlign: 'center' }}>
                        +{pendingHandoverCount - 8} more — view Scheduled Changes tab
                      </div>
                    )}
                  </div>
                )}
              ...
              <div className="giicc-summary-section">
                ...
                {remediationBreakdown.length === 0 ? (
                  <div className="module-empty" style={{ fontSize: 12 }}>
                    No failed or rollback projects.
                  </div>
                ) : (
            ...
            <>
              {error && (
                <div role="alert" style={{ color: 'var(--danger)', padding: 16 }}>
                  ...
                  <button className="btn btn--outline" onClick={refreshBridges} style={{ marginLeft: 8 }}>
                    Retry
                  </button>
                </div>
              )}
              <div className="giicc-board">
                <div className="giicc-col">
                  ...
                  <div className="giicc-col__cards">
                    {activeBridges.length === 0 ? (
                      <div className="giicc-empty">No active bridges</div>
                    ) : (
                      activeBridges.map((b) => (
                        <BridgeCard
                          key={b.cgmp_bridgeid}
                          bridge={b}
                          onExecution={setExecBridge}
                          onPIR={setPirBridge}
                          onEdit={setEditBridge}
                ...
                <div className="giicc-col">
                  ...
                  <div className="giicc-col__cards">
                    {scheduledBridges.length === 0 ? (
                      <div className="giicc-empty">No scheduled bridges</div>
                    ) : (
                      scheduledBridges.map((b) => (
                        <BridgeCard
                          key={b.cgmp_bridgeid}
                          bridge={b}
                          onExecution={setExecBridge}
                ...
                <div className="giicc-col">
                  ...
                  <div className="giicc-col__cards">
                    {todayDone.length === 0 ? (
                      <div className="giicc-empty">None today</div>
                    ) : (
                      todayDone.map((b) => (
                        <BridgeCard
                          key={b.cgmp_bridgeid}
                        ...
                        {bridges.map((b) => {
                          ...
                          return (
                            <tr key={b.cgmp_bridgeid}>
                              <td>{b.cgmp_title}</td>
                              <td>
                                <span className={`badge badge--status ${bridgeBadgeClass(s)}`}>
                                  {bridgeStatusLabel(s)}
                                </span>
                              </td>
                              <td>{TEMPLATE_LABEL[t] ?? '—'}</td>
                              <td>{b.cgmp_changenumber || '—'}</td>
                              <td>{fmtTime(b.cgmp_schedstart)}</td>
                              <td>{fmtTime(b.cgmp_schedend)}</td>
                              <td>{fmtTime(b.cgmp_actualstart)}</td>
                              <td>{fmtTime(b.cgmp_actualend)}</td>
                              <td>{b.cgmp_downtimeminutes != null ? `${b.cgmp_downtimeminutes}m` : '—'}</td>
                              <td>
                                <div style={{ display: 'flex', gap: 4 }}>
                                  <button className="btn btn--xs btn--outline" onClick={() => setEditBridge(b)}>
                                    Edit
                                  </button>
                                  {(s === BRIDGE_STATUS.InProgress || s === BRIDGE_STATUS.Scheduled) && (
                                    <button className="btn btn--xs btn--secondary" onClick={() => setExecBridge(b)}>
                                      Exec
                                    </button>
                                  )}
                                  {(s === BRIDGE_STATUS.InProgress ||
                                    s === BRIDGE_STATUS.Completed ||
                                    s === BRIDGE_STATUS.Failed) && (
                                    <button className="btn btn--xs btn--outline" onClick={() => setPirBridge(b)}>
                                      PIR
                                    </button>
                                  )}
                                </div>
      ...
      <BridgeExecution
        open={!!execBridge}
        bridge={execBridge}
        onClose={() => setExecBridge(null)}
        onUpdated={refreshBridges}
        allChanges={changes}
      />
      <PIRForm
        open={!!pirBridge}
        bridge={pirBridge}
        onClose={() => setPirBridge(null)}
        onUpdated={refreshBridges}
        changeId={changes.find((c) => c.cgmp_changenumber === pirBridge?.cgmp_changenumber)?.cgmp_changeid}
      />
      <ConfirmDialog
        open={!!startTarget}
        onClose={() => setStartTarget(null)}
        onConfirm={async () => {
          await doStartBridge(startTarget!);
          setStartTarget(null);
        }}
        title="Start Bridge"
