...
export default function BridgeExecution({ open, onClose, bridge, onUpdated, allChanges }: Props) {
  ...
  /* G2-2: rollback follow-up change prompt */
  const [showRollbackFollowup, setShowRollbackFollowup] = useState(false);
  const [rollbackFollowupChange, setRollbackFollowupChange] = useState<Cgmp_changes | null>(null);
  ...
  const handleStartBridge = async () => {
    ...
    if (!bridge.cgmp_changenumber) {
      console.warn(
        '[CGMP] Bridge started without a linked change number — actual start time will not be recorded on the parent change and change history will be unavailable'
      );
    }
    ...
    try {
      ...
      const linkedStartChangeId = allChanges.find(
        (c) => c.cgmp_changenumber === bridge.cgmp_changenumber
      )?.cgmp_changeid;
  ...
  const addProjectRow = () => {
    ...
    setProjectStatuses((prev) => [
      ...prev,
      { projectId: unused.cgmp_projectid, projectName: unused.cgmp_name, status: 'On Track', notes: '' },
    ]);
  };
  ...
  const handleSave = async () => {
    ...
    if (actualStart && actualEnd) {
      const s = new Date(actualStart),
        e = new Date(actualEnd);
      if (e <= s) {
    ...
    try {
      const r = await Cgmp_bridgesService.update(bridge.cgmp_bridgeid, {
        ...
        cgmp_projectstatuses: JSON.stringify({
          projects: projectStatuses,
          impactedServices: impactedServices.trim() || undefined,
        }),
      });
  ...
  const handleCompleteBridge = async () => {
    ...
    try {
      ...
      if (rollback && linkedChange) {
        ...
        /* G2-2: prompt to create a follow-up change for rollback tracking */
        setRollbackFollowupChange(linkedChange);
        setShowRollbackFollowup(true);
      }
      ...
      if (bridge.cgmp_changenumber) {
        try {
          ...
          if (allBridgesResult.success && allBridgesResult.data?.length) {
            ...
            if (allTerminal) {
              const anyFailed = fetchedBridges.some(
                (b) => (b.cgmp_status as unknown as number) === BRIDGE_STATUS.Failed
              );
              ...
              if (
                linkedCh &&
                window.confirm(
                  `All bridges for ${bridge.cgmp_changenumber} are complete. Mark change as ${suggestedStatus}?`
                )
              ) {
          ...
          /* ignore — bridge update already succeeded */
        }
      }
  ...
  /* G2-2: create a follow-up change in Draft to track rollback completion */
  const handleRollbackFollowupConfirm = () => {
    if (!rollbackFollowupChange) return;
    Cgmp_changesService.create({
      cgmp_title: `Rollback: ${rollbackFollowupChange.cgmp_title}`,
      cgmp_status: asStatus(100000000), // Draft
      cgmp_risklevel: rollbackFollowupChange.cgmp_risklevel,
      cgmp_category: rollbackFollowupChange.cgmp_category,
      cgmp_parentchangeid: rollbackFollowupChange.cgmp_changeid,
      cgmp_description: `Follow-up rollback change for ${rollbackFollowupChange.cgmp_changenumber}. Original change was rolled back on ${new Date().toISOString()}.`,
    } as any).catch(() => {});
    showToast('success', 'Follow-up rollback change created as Draft.');
    setShowRollbackFollowup(false);
    setRollbackFollowupChange(null);
  };
  ...
  return (
    <>
      {bridge && (
        ...
        >
          <div className="bex-panel">
            {/* Status banner */}
            <div
              className={`bex-status-banner bex-status--${bridgeStatus === BRIDGE_STATUS.InProgress ? 'active' : bridgeStatus === BRIDGE_STATUS.Scheduled ? 'scheduled' : 'done'}`}
            >
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    ...
                    {!actualStart && (
                      <button
                        ...
                        type="button"
                        onClick={() => setActualStart(new Date().toISOString().slice(0, 16))}
                        title="Set to now"
                      >
                        Now
                      </button>
                ...
                <div className="ff-group">
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    ...
                    <button
                      className="btn btn--xs btn--outline"
                      type="button"
                      onClick={() => setActualEnd(new Date().toISOString().slice(0, 16))}
                      title="Set to now"
                    >
                      Now
                    </button>
            ...
            <div className="bex-section">
              <div
                className="bex-section__title"
                style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}
              >
                <span>Project Statuses</span>
                <button className="btn btn--xs btn--outline" onClick={addProjectRow}>
                  + Add Project
                </button>
              </div>
              {projectStatuses.length === 0 ? (
                <div className="bex-empty">
                  No project statuses added. Click "+ Add Project" to track project states.
                </div>
              ) : (
                    ...
                    <div key={i} className="bex-proj-row">
                        ...
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            if (p)
                              setProjectStatuses((prev) =>
                                prev.map((r, idx) =>
                                  idx === i ? { ...r, projectId: p.cgmp_projectid, projectName: p.cgmp_name } : r
                                )
                              );
                          }}
                        >
                          {availableProjects.map((p) => (
                            <option key={p.cgmp_projectid} value={p.cgmp_projectid}>
                              {p.cgmp_name}
                            </option>
                          ))}
                        </select>
                      ...
                      <div className="bex-proj-status">
                        <select
                          className="ff-input ff-select"
                          value={row.status}
                          onChange={(e) => updateProjectRow(i, 'status', e.target.value)}
                        >
                      ...
                      <div className="bex-proj-notes">
                        <input
                          className="ff-input"
                          value={row.notes}
                          onChange={(e) => updateProjectRow(i, 'notes', e.target.value)}
                          placeholder="Notes…"
                        />
                      </div>
            ...
            <div className="bex-section">
              ...
              <span
                className="form-hint"
                style={{ color: 'var(--text-tertiary)', fontSize: 11, marginTop: 4, display: 'block' }}
              >
      ...
      <ConfirmDialog
        open={showRollbackFollowup}
        onClose={() => {
          setShowRollbackFollowup(false);
          setRollbackFollowupChange(null);
        }}
        onConfirm={handleRollbackFollowupConfirm}
        title="Create Follow-up Change"
        message="Create a follow-up change to track rollback completion?"
