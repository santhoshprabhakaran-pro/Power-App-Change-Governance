...
function ValidationRow({ result }: { result: ValidationResult }) {
  ...
  return (
    <div className="rv-check">
      <span className="rv-check__icon" style={{ color }}>{icon}</span>
...
function PenBtn({ onClick, title = 'Edit' }: { onClick: () => void; title?: string }) {
  return (
    <button
      ...
      onClick={onClick}
      title={title}
      style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '1px 3px', color: 'var(--text-tertiary)', display: 'inline-flex', alignItems: 'center', opacity: 0.65, flexShrink: 0 }}
    >
...
function RVField({ label, children, actions }: { label: string; children: React.ReactNode; actions?: React.ReactNode }) {
  return (
...
interface QuickNote { _type: 'itops_note'; text: string; savedBy: string; savedAt: string; }
...
export default function ReviewPanel({ open, onClose, change, allChanges, allProjects, onReviewed }: Props) {
  ...
  useEffect(() => {
    if (!open) { setNotes(''); setNotesError(''); setResolvingIdx(null); setResolutionText(''); setQuickNoteText(''); setEditingField(null); }
  }, [open]);
  ...
  const saveQuickNote = useCallback(async () => {
    ...
    try {
      ...
      const entry: QuickNote = { _type: 'itops_note', text: quickNoteText.trim(), savedBy: currentUserUpn ?? 'IT Ops', savedAt: new Date().toISOString() };
      const r = await Cgmp_changesService.update(change.cgmp_changeid, { cgmp_versionhistory: appendHistory(existingHistory, entry) });
  ...
  const saveFieldEdit = useCallback(async () => {
    ...
    try {
      ...
      else if (editingField === 'impact') updates.cgmp_impactlevel = parseInt(editImpact) as unknown as Cgmp_changescgmp_impactlevel;
      ...
      const r = await Cgmp_changesService.update(change.cgmp_changeid, updates as Parameters<typeof Cgmp_changesService.update>[1]);
      if (!r.success) throw r.error ?? new Error('Save failed');
  ...
  const doResolveConcern = useCallback(async (concernOriginalTimestamp: string) => {
    if (!change || !resolutionText.trim()) return;
    setResolving(true);
    try {
      let history: any[] = [];
      try { history = JSON.parse(change.cgmp_versionhistory ?? '[]'); } catch {}
      ...
      history[idx] = { ...history[idx], resolvedBy: currentUserUpn, resolvedAt: new Date().toISOString(), resolution: resolutionText.trim() };
      const r = await Cgmp_changesService.update(change.cgmp_changeid, { cgmp_versionhistory: JSON.stringify(history) });
      if (!r.success) throw r.error ?? new Error('Save failed');
  ...
  const doReview = useCallback(async (approve: boolean) => {
    if (!change) return;
    ...
    if (!approve && !notes.trim()) { setNotesError('Review notes are required for rejection'); return; }
    if (approve && blocked) { showToast('error', 'Resolve validation errors before approving'); return; }
    ...
    try {
      ...
      showToast(approve ? 'success' : 'info',
        approve ? `${change.cgmp_changenumber} approved and released` : `${change.cgmp_changenumber} rejected and returned to Draft`
      );
      onReviewed();
      onClose();
    } catch (err) {
  ...
  const footer = (
    <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
      <button className="btn btn--outline" onClick={handleClose} disabled={!!working}>Cancel</button>
  ...
  const schedulingConflicts = useMemo(() => {
    ...
    return allChanges.filter(c => {
      ...
      const changeLocs = (change.cgmp_location ?? '').split(',').map(l => l.trim()).filter(Boolean);
      const cLocs = (c.cgmp_location ?? '').split(',').map(l => l.trim()).filter(Boolean);
  ...
  return (
    ...
    >
      <div className="review-panel">
        {/* Validation banner */}
        <div className={`rv-banner ${blocked ? 'rv-banner--error' : warnCount > 0 ? 'rv-banner--warn' : 'rv-banner--ok'}`}>
          {blocked
            ? <><strong>{errorCount} error{errorCount > 1 ? 's' : ''}</strong> must be fixed before approval</>
            : warnCount > 0
              ? <><strong>{warnCount} warning{warnCount > 1 ? 's' : ''}</strong> — approval is allowed</>
        ...
        {change.cgmp_reviewedby && statusCode !== 100000003 /* Released */ && (
          <div className="rv-rereview-banner">
            ...
            <span>
              ...
              {change.cgmp_reviewedat ? ` on ${new Date(change.cgmp_reviewedat).toLocaleDateString(undefined, { timeZone: getDisplayTimezone() })}` : ''}. The change was modified after approval.
            </span>
          </div>
        )}
        ...
        <div className="rv-section">
          ...
          <div className="rv-checks">
            {validation.map(v => <ValidationRow key={v.field} result={v} />)}
          </div>
        </div>
        ...
        <div className="rv-section">
          ...
          <div className="rv-grid">
            ...
            <RVField
              label="Risk"
              actions={editingField === 'risk' ? (
                <>
                  <button className="btn btn--xs btn--primary" onClick={saveFieldEdit} disabled={editSaving} style={{ padding: '1px 6px' }}>{editSaving ? '…' : '✓'}</button>
                  <button className="btn btn--xs btn--ghost" onClick={() => setEditingField(null)} style={{ padding: '1px 6px' }}>✕</button>
                </>
              ) : !editingField ? <PenBtn onClick={() => startEdit('risk')} /> : null}
            >
              {editingField === 'risk' ? (
                <select
            ...
            <RVField label="Category">{change.cgmp_categoryname || categoryLabel(change.cgmp_category as unknown as number)}</RVField>
            <RVField label="Change Type">{change.cgmp_changetypename || changeTypeLabel(change.cgmp_changetype as unknown as number)}</RVField>
            <RVField
              label="Impact"
              actions={editingField === 'impact' ? (
                <>
                  <button className="btn btn--xs btn--primary" onClick={saveFieldEdit} disabled={editSaving} style={{ padding: '1px 6px' }}>{editSaving ? '…' : '✓'}</button>
                  <button className="btn btn--xs btn--ghost" onClick={() => setEditingField(null)} style={{ padding: '1px 6px' }}>✕</button>
                </>
              ) : !editingField ? <PenBtn onClick={() => startEdit('impact')} /> : null}
            >
              {editingField === 'impact' ? (
                <select
                  className="ff-input ff-select"
            ...
            {change.cgmp_isemergency && <RVField label="Emergency"><span className="badge badge--risk risk-critical">Emergency</span></RVField>}
          </div>
        </div>

        {/* Scheduling Conflict Alert (#23) */}
        {schedulingConflicts.length > 0 && (
          <div className="rv-section" style={{ border: '1px solid var(--danger)', borderRadius: 6, padding: 10, background: 'var(--danger-bg, rgba(220,53,69,0.05))' }}>
            <div className="rv-section__title" style={{ color: 'var(--danger)' }}>⚠ Scheduling Conflict — {schedulingConflicts.length} Released Change{schedulingConflicts.length > 1 ? 's' : ''} at Same Location</div>
            {schedulingConflicts.map(c => (
              <div key={c.cgmp_changeid} style={{ fontSize: 12, padding: '4px 0', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ fontWeight: 600 }}>{c.cgmp_changenumber}</span>
                <span style={{ color: 'var(--text-secondary)' }}>{c.cgmp_title?.slice(0, 40)}</span>
                <span style={{ color: 'var(--text-tertiary)' }}>{fmtDate(c.cgmp_starttime)} – {fmtDate(c.cgmp_endtime)}</span>
              </div>
            ))}
          </div>
        )}
        ...
        <div className="rv-section">
          <div className="rv-section__title" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span>Description</span>
            {editingField === 'description' ? (
              <div style={{ display: 'flex', gap: 6 }}>
                <button className="btn btn--xs btn--primary" onClick={saveFieldEdit} disabled={editSaving}>{editSaving ? 'Saving…' : 'Save'}</button>
                <button className="btn btn--xs btn--ghost" onClick={() => setEditingField(null)}>Cancel</button>
              </div>
            ) : !editingField ? <PenBtn onClick={() => startEdit('description')} title="Edit description" /> : null}
          </div>
          {editingField === 'description' ? (
            <textarea
            ...
            <p className="rv-text" style={{ color: change.cgmp_description ? undefined : 'var(--text-tertiary)', fontStyle: change.cgmp_description ? undefined : 'italic' }}>
        ...
        <div className="rv-section">
          <div className="rv-section__title" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            ) : !editingField ? <PenBtn onClick={() => startEdit('projectids')} title="Edit impacted projects" /> : null}
          </div>
          {editingField === 'projectids' ? (
                ...
                {change.cgmp_projectids.split(',').map(s => s.trim()).filter(Boolean).map(id => {
                  ...
                  return (
                    <span key={id} className="rv-text" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor" style={{ color: 'var(--primary)', flexShrink: 0 }}><path d="M20 6h-2.18c.07-.44.18-.88.18-1.34 0-2.58-2.09-4.66-4.66-4.66-1.55 0-2.96.76-3.84 1.94L8 3.62l-1.5-1.68C5.62 1.01 4.22.36 2.66.36 0 .36-2.01 2.44-2.01 5.02c0 .46.11.9.18 1.34H-2v2h22V6z"/></svg>
                      {proj ? proj.cgmp_name : <span style={{ color: 'var(--text-tertiary)', fontStyle: 'italic' }}>{id}</span>}
                    </span>
                  );
                })}
              </div>
            ) : (
              <p className="rv-text" style={{ color: 'var(--text-tertiary)', fontStyle: 'italic', marginTop: 4 }}>No impacted projects</p>
            )
          )}
        </div>
          ...
          return (
            <div className="rv-section" style={{ marginTop: 16 }}>
              <div className="rv-section__title">
                ...
                {unresolvedCount > 0 && <span style={{ marginLeft: 6, color: 'var(--danger)', fontSize: 11 }}>• {unresolvedCount} open</span>}
              </div>
              {concerns.map((ce, i) => (
                <div key={i} style={{
                  fontSize: 12, padding: '8px 10px', borderRadius: 4, marginBottom: 4,
                  background: 'var(--surface-alt)', borderLeft: `3px solid ${ce.resolvedAt ? 'var(--success)' : (CONCERN_COLORS[ce.concernType] ?? 'var(--border)')}`,
                }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                    <div style={{ flex: 1 }}>
                      <strong>{ce.projectName}</strong>{' — '}
                      <span style={{ color: CONCERN_COLORS[ce.concernType] }}>{CONCERN_LABELS[ce.concernType] ?? ce.concernType}</span>
                      {(() => {
                        const effectiveSev = ce.severity ?? CONCERN_TYPE_SEVERITY[ce.concernType];
                        return (
                          <span className={`badge badge--${severityClass(effectiveSev)}`} style={{ fontSize: 10, padding: '1px 5px', marginRight: 4 }}>{severityLabel(effectiveSev)}</span>
                        );
                      })()}
                      ...
                      {resolvingIdx === i && (
                        <div style={{ marginTop: 6, display: 'flex', gap: 6, alignItems: 'flex-start', flexDirection: 'column' }}>
                          <textarea
                            className="ff-input ff-textarea"
                            rows={2}
                          ...
                          />
                          <div style={{ display: 'flex', gap: 6 }}>
                            <button className="btn btn--xs btn--primary" onClick={() => doResolveConcern(ce.timestamp)} disabled={resolving || !resolutionText.trim()}>
                              {resolving ? 'Saving…' : 'Confirm Resolve'}
                            </button>
                            <button className="btn btn--xs btn--ghost" onClick={() => { setResolvingIdx(null); setResolutionText(''); }}>Cancel</button>
                          </div>
                        </div>
                      )}
                    </div>
                    {!ce.resolvedAt && resolvingIdx !== i && (
                      <button className="btn btn--xs btn--outline" onClick={() => { setResolvingIdx(i); setResolutionText(''); }}>
                        Resolve
                      </button>
                    )}
                  </div>
        ...
        {/* IT Ops Quick Notes */}
        <div className="rv-section" style={{ marginTop: 16 }}>
          <div className="rv-section__title">IT Ops Quick Notes <span className="rv-section__hint"> — internal, not visible to PMO</span></div>
          {parseQuickNotes(change.cgmp_versionhistory).map((qn, i) => (
            <div key={i} style={{ fontSize: 12, padding: '6px 10px', background: 'var(--surface-alt)', borderRadius: 4, marginBottom: 6, borderLeft: '3px solid var(--primary)' }}>
              <div style={{ color: 'var(--text-primary)' }}>{qn.text}</div>
              <div style={{ color: 'var(--text-tertiary)', fontSize: 11, marginTop: 2 }}>{qn.savedBy} · {fmtDate(qn.savedAt)}</div>
            </div>
          ))}
          <textarea
            className="ff-input ff-textarea"
