...
import type {
  ...
  Cgmp_notificationsstatecode,
  Cgmp_notificationsBase,
...
function StatChip({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div className="pmo-stat">
      <span className="pmo-stat__value" style={{ color }}>
        {value}
      </span>
...
export default function PMOWorkspace() {
  ...
  const doPublish = useCallback(async () => {
    ...
    try {
      const targets: Cgmp_changes[] = bulkPublish
        ? changes.filter(
            (c) => selectedIds.has(c.cgmp_changeid) && (c.cgmp_status as unknown as number) === STATUS.Draft
          )
        : publishTarget
          ? [publishTarget]
          : [];
      ...
      try {
        ...
        /* don't block publish on blackout fetch failure */
      }
      ...
      setOptimisticUpdates((prev) => {
        const next = { ...prev };
        targets.forEach((c) => {
          next[c.cgmp_changeid] = optimisticPatch;
        });
        return next;
      });
      ...
      if (failedNums.length > 0) {
        setOptimisticUpdates((prev) => {
          const next = { ...prev };
          settleResults.forEach((r, i) => {
            if (r.status === 'rejected') delete next[targets[i].cgmp_changeid];
          });
          return next;
        });
      }

      /* Send notifications to IT Ops POCs for successfully published changes (fire-and-forget — never awaited) */
      const itOpsPocs = allProfiles.filter((p) => (p.cgmp_role as unknown as number) === 100000002);
      // G2-8: track publish events
      succeeded.forEach((c) => {
        trackAppEvent('change.published', { changeNumber: c.cgmp_changenumber ?? '' });
      });
      succeeded.forEach((c) => {
        ...
        const recipients = itOpsPocs.filter((p) => {
          const pocLocs = ((p as any).cgmp_assignedlocations ?? '')
            .split(',')
            .map((l: string) => l.trim())
            .filter(Boolean);
        ...
        recipients.forEach((p) => {
          // G2-7: skip if recipient opted out of Review Request notifications
          if (!isNotifCategoryEnabled(p.cgmp_notificationcategories, 100000000)) return;
          // G2-6: apply quiet-hours snooze when recipient is in their quiet window
          const snoozedUntil = getQuietHoursSnoozedUntil(p.cgmp_quiethoursstart, p.cgmp_quiethoursend);
      ...
      if (noRecipients.length > 0) {
        showToast(
          'warning',
          `${noRecipients.length} change${noRecipients.length > 1 ? 's have' : ' has'} no matching IT Ops POC — no notification sent. Assign a POC for: ${noRecipients.map((c) => c.cgmp_changenumber).join(', ')}`
        );
      }
      if (failedNums.length > 0) {
      ...
      showToast(
        ...
        err instanceof Error
          ? err.message
          : 'Publish failed — some changes may not have been published. Please check the list.'
      );
    } finally {
      setPublishing(false);
      if (!earlyReturn) {
  ...
  const doPublishWithOverride = useCallback(async () => {
    ...
    try {
      ...
      await Promise.allSettled(
        pendingPublishState.overlapping.map(async (c) => {
          try {
  ...
  const eligibleDraftSelected = changes.filter(
    (c) => selectedIds.has(c.cgmp_changeid) && (c.cgmp_status as unknown as number) === STATUS.Draft
  ).length;
  ...
  return (
    <div className="pmo-workspace">
      ...
      <div className="pmo-stats">
        ...
        {selectedIds.size > 0 && <span className="pmo-selection-chip">{selectedIds.size} selected</span>}
      </div>
      ...
      {/* Bulk Import dialog */}
      <BulkImport open={importOpen} onClose={() => setImportOpen(false)} onImported={refresh} />
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          >
            This change window overlaps an active blackout period. As Admin, you may override but must provide a
            justification that will be recorded in the change history.
          </div>
