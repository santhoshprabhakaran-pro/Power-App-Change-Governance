...
import { Cgmp_changesService, Cgmp_auditlogsService, Cre09_knowledgebasearticlesService, Cgmp_blackoutperiodsService, Cgmp_notificationsService, Cgmp_changehistoryService } from '../../generated';
...
import type { Cgmp_changescgmp_changetype, Cgmp_changescgmp_impactlevel } from '../../generated/models/Cgmp_changesModel';
import type { Cgmp_auditlogsBase } from '../../generated/models/Cgmp_auditlogsModel';
import { asStatus, asRisk, asCategory, asAuditEventType, asAuditEntityType, asNotifCategory, asNotifPriority } from '../../utils/optionSets';
...
import { statusLabel, riskLabel, useProjects, useSystemUsers, useChangeList, useAllUserProfiles } from '../../hooks/useDataverse';
...
function blankForm(): FormState {
  return {
    title: '', changenumber: '', owner: '', changepoc: '',
    status: '100000000', category: '', changetype: '',
    risklevel: '', impactlevel: '',
    starttime: '', endtime: '',
    isemergency: false, isweekend: false, uatrequired: false,
    description: '', timeline: '', location: '', region: '', country: '', facility: '',
    projectids: '', pirnotes: '', parentchangeid: '',
    closureremarks: '',
  };
}
    ...
    <span style={{ position: 'relative', display: 'inline-flex', verticalAlign: 'middle', marginLeft: 4 }}>
      ...
      {show && (
        <div className="help-tip-popover" role="tooltip">
          {text}
          <button type="button" className="help-tip-close" onClick={() => setShow(false)} aria-label="Close help">×</button>
        </div>
      )}
    </span>
  );
...
function RelatedKBArticles({ changeId, categoryCode }: { changeId: string; categoryCode: number | undefined }) {
  ...
  useEffect(() => {
    let cancelled = false;
    Cre09_knowledgebasearticlesService.getAll({ top: 100, filter: 'cre09_cgmp_ispublished eq true' }).then(res => {
      if (cancelled) return;
      const all = res.data ?? [];
      const matched = all.filter(a => {
        ...
        return tags.split(',').map(t => t.trim()).some(t => t === catLabel || t.includes(catLabel));
      });
      setArticles(matched);
      setLoaded(true);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      {articles.map(a => (
        <div key={a.cre09_knowledgebasearticleid} style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontWeight: 600, fontSize: 13, color: 'var(--text-primary)' }}>{a.cre09_cgmp_title ?? '—'}</span>
          ...
          {a.cre09_cgmp_tags && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
              {a.cre09_cgmp_tags.split(',').map(t => t.trim()).filter(Boolean).map(t => (
...
export default function ChangeForm({ open, onClose, mode, change, onSaved, initialTemplate }: Props) {
  ...
  useEffect(() => {
    Cgmp_blackoutperiodsService.getAll({ top: 100 }).then(r => setBlackoutPeriods(r.data ?? [])).catch(err => { if (import.meta.env.DEV) console.error('Background operation failed:', err); });
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (raw) {
        const saved = JSON.parse(raw) as { form: FormState; versionHistory?: string; savedAt: number };
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      try {
        localStorage.setItem(draftKey, JSON.stringify({ form, versionHistory: '[]', savedAt: Date.now() }));
      } catch {}
    }, 30000);
    return () => clearInterval(id);
  }, [open, mode, form, draftKey]);
  ...
  const schedulingConflicts = useMemo(() => {
    ...
    const formLocs = form.location.split(',').map(l => l.trim().toLowerCase()).filter(Boolean);
    ...
    return allChanges.filter(c => {
      ...
      const cLocs = (c.cgmp_location ?? '').split(',').map(l => l.trim().toLowerCase()).filter(Boolean);
  ...
  useEffect(() => {
    ...
    const checkAndLock = async () => {
      try {
        ...
        } as unknown as Omit<Cgmp_auditlogsBase, 'cgmp_auditlogid'>).catch(err => { if (import.meta.env.DEV) console.error('Audit log failed:', err); }); // SDK requires typed cast
      } catch { /* non-fatal */ }
    };
  ...
  const set = useCallback((key: keyof FormState, value: string | boolean) => {
    setForm(prev => {
      ...
      if (key === 'region' && typeof value === 'string') {
        ...
        selectedRegions.forEach(r => { const rc = regionToCountries.get(r); if (rc) rc.forEach(c => available.add(c)); });
        if (available.size > 0) {
          const validCountries = prev.country.split(',').filter(Boolean).filter(c => available.has(c));
  ...
  const handleSave = useCallback(async () => {
    ...
    if (Object.keys(errs).length > 0) {
      ...
      setValidationSummary(`Form has ${errCount} error${errCount !== 1 ? 's' : ''}. Please review the highlighted fields.`);
      const fieldIdMap: Record<string, string> = {
        title: 'cf-title', changenumber: 'cf-chgnum', category: 'cf-cat',
        changetype: 'cf-type', risklevel: 'cf-risk', impactlevel: 'cf-impact',
        starttime: 'cf-start', endtime: 'cf-end',
      };
    ...
    try {
      const base = ({
        ...
        cgmp_parentchangeid: form.parentchangeid || undefined,
      ...
      if (mode === 'create') {
        ...
        if (created?.cgmp_changeid) {
          ...
          Cgmp_changehistoryService.create({
            ...
            cgmp_details: JSON.stringify({ action: 'created', changedFields: (Object.keys(form) as (keyof FormState)[]).filter(k => !!form[k]) }),
        ...
        const latestChange = await Cgmp_changesService.get(change.cgmp_changeid, { select: ['cgmp_versionhistory'] as any });
        const prevHistory = JSON.parse((latestChange.data?.cgmp_versionhistory ?? change.cgmp_versionhistory) ?? '[]') as object[];
        ...
        if ((effectiveStatus === STATUS.Closed || effectiveStatus === STATUS.Completed) && change.cgmp_actualstarttime) {
        ...
        const updateResult = await Cgmp_changesService.update(change.cgmp_changeid, updatePayload as Parameters<typeof Cgmp_changesService.update>[1]);
        ...
        Cgmp_auditlogsService.create({
          ...
          cgmp_newvalues: JSON.stringify({ title: form.title, status: isPostRelease ? 'Published (Re-Review Required)' : form.status }),
        } as unknown as Omit<Cgmp_auditlogsBase, 'cgmp_auditlogid'>).catch(err => { if (import.meta.env.DEV) console.error('Audit log failed:', err); }); // SDK requires typed cast
        showToast('success', isPostRelease
        ...
        if (form.isemergency && effectiveStatus === STATUS.Published) {
          allProfiles
            ...
            .forEach((admin: any) => {
              ...
              } as any).catch(() => {}); // SDK requires untyped cast — notification fields partially absent from generated type
            });
        }
      }
  ...
  useEffect(() => {
    ...
    const timer = setInterval(async () => {
      ...
      try {
        ...
        const r = await Cgmp_changesService.update(c.cgmp_changeid, payload as Parameters<typeof Cgmp_changesService.update>[1]);
  ...
  const pendingProposals = useMemo(() => {
    ...
    let hist: any[] = [];
    try { hist = JSON.parse(change.cgmp_versionhistory); } catch { return []; }
    const resolved = new Set(
      hist.filter((e: any) => e._type === 'rescheduleAccepted' || e._type === 'rescheduleDeclined')
  ...
  const handleAcceptReschedule = useCallback(async (proposal: any) => {
    if (!change) return;
    setReschedWorking(true);
    try {
      const latestChange = await Cgmp_changesService.get(change.cgmp_changeid, { select: ['cgmp_versionhistory'] as any });
      let hist: any[] = [];
      try { hist = JSON.parse((latestChange.data?.cgmp_versionhistory ?? change.cgmp_versionhistory) ?? '[]'); } catch {}
      const alreadyActioned = hist.some((e: any) => (e._type === 'rescheduleAccepted' || e._type === 'rescheduleDeclined') && e.proposedTs === proposal.timestamp);
      if (alreadyActioned) { showToast('info', 'This proposal was already actioned by another user.'); return; }
      hist.push({ _type: 'rescheduleAccepted', proposedTs: proposal.timestamp, by: currentUserUpn, timestamp: new Date().toISOString(), prevStart: change.cgmp_starttime, prevEnd: change.cgmp_endtime });
      ...
      const proposer = allProfiles.find((p: any) => (proposal.byProfileId && p.cgmp_userprofileid === proposal.byProfileId) || p.cgmp_userprincipalname === proposal.by);
      if (proposer) {
        Cgmp_notificationsService.create({
          ...
          statecode: 0 as any, // SDK requires untyped cast — statecode not in generated type
        } as any).catch(err => { if (import.meta.env.DEV) console.error('Background operation failed:', err); }); // SDK requires untyped cast
      }
  ...
  const handleDeclineReschedule = useCallback(async (proposal: any) => {
    if (!change) return;
    setReschedWorking(true);
    try {
      const latestChange = await Cgmp_changesService.get(change.cgmp_changeid, { select: ['cgmp_versionhistory'] as any });
      let hist: any[] = [];
      try { hist = JSON.parse((latestChange.data?.cgmp_versionhistory ?? change.cgmp_versionhistory) ?? '[]'); } catch {}
      const alreadyActionedDecline = hist.some((e: any) => (e._type === 'rescheduleAccepted' || e._type === 'rescheduleDeclined') && e.proposedTs === proposal.timestamp);
      if (alreadyActionedDecline) { showToast('info', 'This proposal was already actioned by another user.'); return; }
      hist.push({ _type: 'rescheduleDeclined', proposedTs: proposal.timestamp, by: currentUserUpn, timestamp: new Date().toISOString() });
      const r = await Cgmp_changesService.update(change.cgmp_changeid, {
        cgmp_versionhistory: JSON.stringify(hist),
      } as any);
  ...
  const title = mode === 'create' ? 'Create Change' : mode === 'edit' ? `Edit: ${form.changenumber}` : form.changenumber;
  ...
  const footer = mode !== 'view' ? (
    <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', alignItems: 'center' }}>
      ...
      <button className="btn btn--outline" onClick={handleClose} disabled={saving}>Cancel</button>
      <button className="btn btn--primary" onClick={handleSave} disabled={saving || (!!concurrentEditor && mode === 'edit')}>
        {saving ? 'Saving…' : mode === 'create' ? 'Create Change' : 'Save Changes'}
      </button>
    </div>
  ) : (
    <button className="btn btn--outline" style={{ width: '100%' }} onClick={handleClose}>Close</button>
  );
  ...
  return (
    ...
    <SlidePanel open={open} onClose={handleClose} title={title} subtitle={subtitle} width={560} footer={footer}>
      {draftRestoreVisible && mode === 'create' && (
        <div role="alert" style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 16px', background: 'var(--warning-light, #fffbeb)', border: '1px solid var(--warning, #d97706)', borderRadius: 6, marginBottom: 8, fontSize: 13 }}>
          <span style={{ flex: 1 }}>Unsaved draft found (saved at {draftSavedAt ? new Date(draftSavedAt).toLocaleTimeString(undefined, { timeZone: getDisplayTimezone() }) : ''}). Restore it?</span>
          <button className="btn btn--xs btn--primary" onClick={() => {
            try {
              if (!draftKey) return;
              const raw = localStorage.getItem(draftKey);
              if (raw) { const saved = JSON.parse(raw) as { form: FormState }; if (saved?.form) { setForm(saved.form); setIsDirty(true); } }
            } catch {}
            setDraftRestoreVisible(false);
          }}>Restore</button>
      ...
      {concurrentEditor && mode === 'edit' && (
        <div className="concurrent-edit-banner concurrent-edit-banner--block" role="alert" style={{ flexDirection: 'column', gap: 8, alignItems: 'flex-start', padding: '12px 16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" style={{ color: 'var(--danger)', flexShrink: 0 }}>
              <path d="M18 8h-1V6c0-2.76-2.24-5-5-5S7 3.24 7 6v2H6c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2zm-6 9c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2zm3.1-9H8.9V6c0-1.71 1.39-3.1 3.1-3.1 1.71 0 3.1 1.39 3.1 3.1v2z" />
            </svg>
            <span><strong>{concurrentEditor}</strong> is currently editing this change. Editing is blocked to prevent conflicts.</span>
          </div>
          <button type="button" className="btn btn--xs btn--outline" style={{ borderColor: 'currentColor' }} onClick={() => setConcurrentEditor(null)}>
      ...
      {pendingProposals.length > 0 && change && (
        <div role="alert" style={{ margin: '0 0 16px', padding: '12px 16px', background: '#FFF3CD', border: '1px solid #FFD700', borderRadius: 6 }}>
          {pendingProposals.map((p: any, i: number) => {
            ...
            return (
              <div key={p.timestamp ?? i} style={{ marginBottom: i < pendingProposals.length - 1 ? 12 : 0 }}>
                <div style={{ fontWeight: 600, fontSize: 13, marginBottom: 6 }}>⏱ IT Ops has proposed a new time window</div>
                <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: canAct ? 10 : 0, lineHeight: 1.8 }}>
                  <strong>Proposed by:</strong> {p.by}<br />
                  <strong>New Start:</strong> {fmtDateTime(p.proposedStart)}<br />
                  <strong>New End:</strong> {fmtDateTime(p.proposedEnd)}<br />
                  {p.reason && <><strong>Reason:</strong> {p.reason}</>}
                </div>
                {canAct && (
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button className="btn btn--xs btn--primary" onClick={() => handleAcceptReschedule(p)} disabled={reschedWorking}>
                      {reschedWorking ? 'Processing…' : `Accept & Apply Schedule (${change.cgmp_changenumber})`}
                    </button>
                    <button className="btn btn--xs btn--outline" onClick={() => handleDeclineReschedule(p)} disabled={reschedWorking}>
                      Decline
                    </button>
                  </div>
                )}
        ...
        {mode === 'view' && change ? (
          ...
          <>
            {change.cgmp_isemergency && (
              <div className="banner banner--danger" style={{ marginBottom: 12, padding: '8px 14px', background: 'var(--danger-faint, rgba(220,53,69,0.08))', border: '1px solid var(--danger)', borderRadius: 6, color: 'var(--danger)', fontWeight: 600, fontSize: 13 }}>
                ...
                <span className="view-field__value" style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                  ...
                  {change.cgmp_changenumber && (
                    <button
                      ...
                      onClick={e => { e.stopPropagation(); navigator.clipboard.writeText(change.cgmp_changenumber ?? ''); showToast('success', 'Copied!'); }}
                      style={{ padding: '2px 6px', fontSize: 10 }}
                    >&#10697;</button>
                  )}
                </span>
              </div>
              <ReadField label="Status" value={change.cgmp_statusname || statusLabel(change.cgmp_status as unknown as number)} />
            ...
            <div className="view-grid">
              <ReadField label="Category" value={change.cgmp_categoryname || categoryLabel(change.cgmp_category as unknown as number)} />
              <ReadField label="Change Type" value={change.cgmp_changetypename || changeTypeLabel(change.cgmp_changetype as unknown as number)} />
              <ReadField label="Risk Level" value={change.cgmp_risklevelname || riskLabel(change.cgmp_risklevel as unknown as number)} />
              <ReadField label="Impact Level" value={change.cgmp_impactlevelname || impactLabel(change.cgmp_impactlevel as unknown as number)} />
            </div>
            ...
            <div className="view-grid">
              <div className="view-field">
                ...
                <span style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                  {change.cgmp_location
                    ? change.cgmp_location.split(',').map(l => l.trim()).filter(Boolean).map(loc => (
              ...
              <div className="view-field">
                ...
                <span style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                  {change.cgmp_region ? change.cgmp_region.split(',').filter(Boolean).map(r => (
                    <span key={r} className="badge">{r.trim()}</span>
                  )) : <span className="view-field__value">—</span>}
                </span>
              </div>
              <div className="view-field">
                ...
                <span style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                  {change.cgmp_country ? change.cgmp_country.split(',').filter(Boolean).map(c => (
                    <span key={c} className="badge">{c.trim()}</span>
                  )) : <span className="view-field__value">—</span>}
                </span>
              </div>
              <div className="view-field">
                ...
                <span style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                  {change.cgmp_facility
                    ? change.cgmp_facility.split(',').map(f => f.trim()).filter(Boolean).map(fac => (
                        <span key={fac} className="badge">{fac}</span>
                      ))
                    : <span className="view-field__value">—</span>
                  }
                </span>
            ...
            {change.cgmp_projectids && (
              <>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 4 }}>
                  {change.cgmp_projectids.split(',').map(s => s.trim()).filter(Boolean).map(id => (
            ...
            {change.cgmp_parentchangeid && (
              <>
                ...
                {(() => {
                  const parent = (allChanges ?? []).find((c: Cgmp_changes) => c.cgmp_changeid === change.cgmp_parentchangeid);
                  return (
            ...
            {(change.cgmp_lessonslearned || change.cgmp_closureremarks || change.cgmp_downtimeminutes != null || change.cgmp_rollbackinitiated) && (
              <>
                <FormSection title="Post-Implementation" />
                <div className="view-grid">
                  {change.cgmp_downtimeminutes != null && <ReadField label="Downtime (min)" value={String(change.cgmp_downtimeminutes)} />}
                  {change.cgmp_rollbackinitiated != null && <ReadField label="Rollback Initiated" value={change.cgmp_rollbackinitiated} />}
                </div>
                {change.cgmp_lessonslearned && <p className="view-text" style={{ marginTop: 6 }}><strong style={{ fontSize: 11, color: 'var(--text-tertiary)', textTransform: 'uppercase' }}>Lessons Learned</strong><br />{change.cgmp_lessonslearned}</p>}
                {change.cgmp_closureremarks && <p className="view-text"><strong style={{ fontSize: 11, color: 'var(--text-tertiary)', textTransform: 'uppercase' }}>Closure Remarks</strong><br />{change.cgmp_closureremarks}</p>}
              </>
            )}
            ...
            {(() => {
              ...
              try {
                ...
                if (pub) { publishedDate = pub.timestamp; publishedBy = pub.user; }
              } catch { /* noop */ }
              const steps = [
                { label: 'Draft', done: s > 100000000, date: change.createdon, by: change.createdbyname ?? undefined },
                { label: 'Published', done: s > 100000001, date: publishedDate, by: publishedBy },
                { label: 'Under Review', done: s > 100000002, date: (change as any).cgmp_reviewedat, by: (change as any).cgmp_reviewedby },
                { label: 'Released', done: s >= 100000003, date: (change as any).cgmp_releasedat, by: (change as any).cgmp_releasedby },
              ];
              ...
              return (
                <>
                  ...
                  <div className="approval-chain">
                    {steps.map((st, i) => (
                      <div key={st.label} className={`approval-step ${st.done ? 'approval-step--done' : i === activeIdx + 1 ? 'approval-step--active' : 'approval-step--pending'}`}>
                        {i > 0 && <div className={`approval-connector ${steps[i - 1].done ? 'approval-connector--done' : ''}`} />}
                        ...
                        <div className="approval-step__info">
                          ...
                          {st.date && <span className="approval-step__date">{new Date(st.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric', timeZone: getDisplayTimezone() })}</span>}
            ...
            <div className="view-grid" style={{ marginBottom: 8 }}>
              ...
              {change.cgmp_releasedat && <ReadField label="Released At" value={fmtDateTime(change.cgmp_releasedat)} />}
              {change.cgmp_releasedby && <ReadField label="Released By" value={change.cgmp_releasedby} />}
              {change.cgmp_reviewedat && <ReadField label="Reviewed At" value={fmtDateTime(change.cgmp_reviewedat)} />}
              {change.cgmp_reviewedby && <ReadField label="Reviewed By" value={change.cgmp_reviewedby} />}
            </div>

            <FormSection title="Version History" />
            <VersionHistory json={change.cgmp_versionhistory} currentValues={fromChange(change) as unknown as Record<string, unknown>} />
          ...
          <>
            ...
            {mode === 'edit' && change && (change.cgmp_status as unknown as number) >= STATUS.Released && (
              <div className="cf-rereview-banner">
                <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor" style={{ flexShrink: 0, color: 'var(--orange)' }}>
                  <path d="M1 21h22L12 2 1 21zm12-3h-2v-2h2v2zm0-4h-2v-4h2v4z" />
                </svg>
                <span>
                  <strong>Approval will be revoked.</strong> This change has already been approved by IT Ops.
                  Saving changes will automatically return it to the review queue and require re-approval before it can be released again.
                </span>
              </div>
            )}
            <fieldset disabled={!!concurrentEditor && mode === 'edit'} style={{ border: 'none', padding: 0, margin: 0 }}>
            <FormSection title="Basic Information" />
            <Field label="Title" required error={errors.title} htmlFor="cf-title">
              <Input id="cf-title" value={form.title} onChange={e => set('title', e.target.value)} error={!!errors.title} placeholder="Brief description of the change" />
            </Field>
            <FormRow>
              ...
              {mode === 'edit' && (() => {
                ...
                return (
                  <Field label="Status" htmlFor="cf-status">
                    <Select id="cf-status" value={form.status} onChange={e => set('status', e.target.value)} options={statusOptions} />
                  </Field>
                );
              })()}
            </FormRow>
            ...
            <Field label="Change Owner" htmlFor="cf-owner">
              <UserPickerSelect
                ...
                value={form.owner}
                onChange={name => { setForm(prev => ({ ...prev, owner: name })); setIsDirty(true); }}
              />
            ...
            <Field label="Change POC" htmlFor="cf-changepoc">
              <UserPickerSelect
                ...
                onChange={name => { setForm(prev => ({ ...prev, changepoc: name })); setIsDirty(true); }}
              />
            ...
            <FormRow>
              <Field label="Category" required error={errors.category} htmlFor="cf-cat">
                <Select id="cf-cat" value={form.category} onChange={e => set('category', e.target.value)} options={CATEGORY_OPTIONS} placeholder="Select category" error={!!errors.category} />
              </Field>
              <Field label="Change Type" required error={errors.changetype} htmlFor="cf-type">
                <Select id="cf-type" value={form.changetype} onChange={e => set('changetype', e.target.value)} options={CHANGE_TYPE_OPTIONS} placeholder="Select type" error={!!errors.changetype} />
              </Field>
            </FormRow>
            <FormRow>
              <Field label="Risk Level" required error={errors.risklevel} htmlFor="cf-risk">
                <Select id="cf-risk" value={form.risklevel} onChange={e => set('risklevel', e.target.value)} options={RISK_OPTIONS} placeholder="Select risk" error={!!errors.risklevel} />
              </Field>
              <Field label="Impact Level" required error={errors.impactlevel} htmlFor="cf-impact">
                <Select id="cf-impact" value={form.impactlevel} onChange={e => set('impactlevel', e.target.value)} options={IMPACT_OPTIONS} placeholder="Select impact" error={!!errors.impactlevel} />
              </Field>
            </FormRow>
            ...
            <FormRow>
              <Field label="Start Date / Time" required error={errors.starttime} htmlFor="cf-start">
                <DateTimeInput id="cf-start" value={form.starttime} onChange={e => set('starttime', e.target.value)} error={!!errors.starttime} />
              </Field>
              <Field label="End Date / Time" required error={errors.endtime} htmlFor="cf-end">
                <DateTimeInput id="cf-end" value={form.endtime} onChange={e => set('endtime', e.target.value)} error={!!errors.endtime} />
              </Field>
            </FormRow>
            {blackoutWarning && (
              <div className="cf-conflict-banner" role="alert" style={{ borderColor: 'var(--warning, #d97706)', background: 'var(--warning-light, #fffbeb)', color: 'var(--warning-text, #92400e)' }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" style={{ color: 'var(--warning, #d97706)', flexShrink: 0 }}><path d="M1 21h22L12 2 1 21zm12-3h-2v-2h2v2zm0-4h-2v-4h2v4z" /></svg>
                <div>
                  <strong>Blackout Period Warning</strong> — This change overlaps a scheduled freeze period:
                  <div style={{ fontSize: 12, marginTop: 4 }}>
                    {blackoutWarning.cgmp_name || 'Blackout Period'}: {blackoutWarning.cgmp_startdate?.slice(0, 10)} – {blackoutWarning.cgmp_enddate?.slice(0, 10)}
            ...
            {schedulingConflicts.length > 0 && (
              <div className="cf-conflict-banner" role="alert">
                ...
                <div>
                  <strong>Scheduling conflict</strong> — {schedulingConflicts.length} change{schedulingConflicts.length > 1 ? 's' : ''} overlap at the same location during this window:
                  <ul style={{ margin: '4px 0 0', paddingLeft: 16 }}>
                    {schedulingConflicts.slice(0, 3).map(c => (
                      <li key={c.cgmp_changeid} style={{ fontSize: 12 }}>
                        {c.cgmp_changenumber ?? '—'} — {c.cgmp_title ?? '—'} ({statusLabel(c.cgmp_status as unknown as number)})
                      </li>
                    ))}
                    {schedulingConflicts.length > 3 && <li style={{ fontSize: 12 }}>…and {schedulingConflicts.length - 3} more</li>}
                  </ul>
                </div>
              </div>
            )}
            <div style={{ display: 'flex', gap: 24, marginTop: 4 }}>
              <Toggle
                ...
                id="cf-emergency"
              />
              <Toggle checked={form.isweekend} onChange={v => set('isweekend', v)} label="Weekend Change" id="cf-weekend" />
              <Toggle checked={form.uatrequired} onChange={v => set('uatrequired', v)} label="UAT Required" id="cf-uatrequired" />
            </div>
            ...
            <FormRow>
              <Field label="Location" htmlFor="cf-loc">
                {locationOptions.length > 0 ? (
                  <div className="multi-loc-list" style={{ maxHeight: 120, overflowY: 'auto', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '6px 8px' }}>
                      ...
                      <label key={opt.value} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '2px 0', cursor: 'pointer' }}>
                        <input
                          ...
                          onChange={e => {
                            ...
                            const next = e.target.checked ? [...current, opt.value] : current.filter(l => l !== opt.value);
                  ...
                  <Input id="cf-loc" value={form.location} onChange={e => set('location', e.target.value)} placeholder="e.g. Chennai" />
                )}
              </Field>
              <Field label="Region" htmlFor="cf-region">
                {regionOptions.length > 0 ? (
                      ...
                      <label key={opt.value} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '2px 0', cursor: 'pointer' }}>
                        <input
                          ...
                          onChange={e => {
                            ...
                            const next = e.target.checked ? [...current, opt.value] : current.filter(r => r !== opt.value);
                  ...
                  <Input id="cf-region" value={form.region} onChange={e => set('region', e.target.value)} placeholder="e.g. APAC" />
                )}
              </Field>
            </FormRow>
            <FormRow>
                      ...
                      <label key={opt.value} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '2px 0', cursor: 'pointer' }}>
                        <input
                          ...
                          onChange={e => {
                            ...
                            const next = e.target.checked ? [...current, opt.value] : current.filter(c => c !== opt.value);
              ...
              <Field label="Facility" htmlFor="cf-facility">
                {facilityOptions.length > 0 ? (
                      ...
                      <label key={opt.value} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '2px 0', cursor: 'pointer' }}>
                        <input
                          ...
                          onChange={e => {
                            ...
                            const next = e.target.checked ? [...current, opt.value] : current.filter(f => f !== opt.value);
                  ...
                  <Input id="cf-facility" value={form.facility} onChange={e => set('facility', e.target.value)} placeholder="e.g. Building 3" />
                )}
              </Field>
            </FormRow>

            <FormSection title="Description" />
            <Textarea value={form.description} onChange={e => set('description', e.target.value)} placeholder="Detailed description of the change, impact, and rollback plan…" rows={4} maxLength={2000} />
            <div className={`ff-char-count${form.description.length > 1900 ? ' ff-char-count--error' : form.description.length > 1600 ? ' ff-char-count--warn' : ''}`} aria-live="polite">
              {form.description.length} / 2000
            </div>

            <FormSection title="Timeline" />
            <Textarea value={form.timeline} onChange={e => set('timeline', e.target.value)} placeholder="High-level implementation timeline and key milestones…" rows={3} maxLength={1000} />
            <div className={`ff-char-count${form.timeline.length > 950 ? ' ff-char-count--error' : form.timeline.length > 800 ? ' ff-char-count--warn' : ''}`} aria-live="polite">
            ...
            <Field
              ...
              htmlFor="cf-proj"
              labelSuffix={<HelpTip text="Select all projects impacted by this change. ISM teams for each project will be notified." />}
            >
              <ProjectMultiSelect
                projects={projects}
            ...
            >
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                <input
                  ...
                  value={parentSearch || (form.parentchangeid && !parentSearch ? (allChanges.find(c => c.cgmp_changeid === form.parentchangeid)?.cgmp_changenumber ?? form.parentchangeid) : '')}
                  onChange={e => { setParentSearch(e.target.value); if (!e.target.value) set('parentchangeid', ''); }}
                />
                {parentSearch && (
                  <div style={{ border: '1px solid var(--border)', borderRadius: 4, maxHeight: 180, overflowY: 'auto', background: 'var(--surface)' }}>
                    {parentOpts.length === 0 ? (
                      <div style={{ padding: '8px 12px', fontSize: 12, color: 'var(--text-tertiary)' }}>No matches</div>
                    ) : parentOpts.map(c => (
                      <div
                        key={c.cgmp_changeid}
                        ...
                        onClick={() => { set('parentchangeid', c.cgmp_changeid); setParentSearch(''); }}
                      >
                ...
                {form.parentchangeid && !parentSearch && (
                  <button className="btn btn--xs btn--danger-outline" style={{ alignSelf: 'flex-start' }} onClick={() => set('parentchangeid', '')}>
                    Clear
                  </button>
                )}
              </div>
            ...
            {mode === 'edit' && (
              <>
                ...
                <Field
                  label="PIR Notes"
                  htmlFor="cf-pirnotes"
                  labelSuffix={<HelpTip text="Post-Implementation Review notes. Describe what went well, what failed, and key learnings." />}
                >
                  <Textarea id="cf-pirnotes" value={form.pirnotes} onChange={e => set('pirnotes', e.target.value)} rows={3} placeholder="Post-implementation review notes…" maxLength={5000} />
                  <div className={`ff-char-count${form.pirnotes.length > 4750 ? ' ff-char-count--error' : form.pirnotes.length > 4000 ? ' ff-char-count--warn' : ''}`} aria-live="polite">
            ...
            {mode === 'edit' && (
              <>
                ...
                <Field
                  ...
                  htmlFor="cf-closureremarks"
                >
                  <Textarea id="cf-closureremarks" value={form.closureremarks} onChange={e => set('closureremarks', e.target.value)} rows={3} placeholder="Closure remarks and post-implementation summary…" maxLength={2000} />
                  <div className={`ff-char-count${form.closureremarks.length > 1900 ? ' ff-char-count--error' : form.closureremarks.length > 1600 ? ' ff-char-count--warn' : ''}`} aria-live="polite" aria-atomic="true">
    ...
    <ConfirmDialog
      ...
      variant="destructive"
      onConfirm={() => { setNavGuardOpen(false); setIsDirty(false); onClose(); }}
    ...
    <ConfirmDialog
      ...
      cancelLabel="Cancel"
      variant="destructive"
      onConfirm={() => { setBlackoutConfirmOpen(false); blackoutConfirmResolveRef.current?.(true); }}
      onClose={() => { setBlackoutConfirmOpen(false); blackoutConfirmResolveRef.current?.(false); }}
    />
    </>
  );
}
