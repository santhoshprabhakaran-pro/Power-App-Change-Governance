...
const ROLE_CLASS: Record<number, string> = {
  ...
  100000006: 'status-published',
  100000007: 'status-released',
};
...
export default function SecurityRoles() {
  ...
  const [secTab, setSecTab] = useState<'roles' | 'activity' | 'itopspocs' | 'events'>(() => {
    try {
      return (
        (localStorage.getItem('cgmp-tab-security') as 'roles' | 'activity' | 'itopspocs' | 'events' | null) ?? 'roles'
      );
    } catch {
      return 'roles';
    }
  });
  const handleSecTabChange = useCallback((val: 'roles' | 'activity' | 'itopspocs' | 'events') => {
    try {
      localStorage.setItem('cgmp-tab-security', val);
    } catch {}
    setSecTab(val);
  }, []);
  ...
  const doAddUsers = useCallback(async () => {
    ...
    try {
      if (addMode === 'individual') {
        ...
        if (toAdd.length === 0) {
          showToast('error', 'No resolved members to add');
          return;
        }
        for (const m of toAdd) {
          const upn = m.user!.internalemailaddress || m.user!.domainname || m.upn;
          try {
            await createProfile(upn, m.user!.fullname ?? upn, roleVal);
          } catch (e) {
            errs.push(e instanceof Error ? e.message : upn);
          }
        }
        if (errs.length === 0)
          showToast(
            'success',
            `Added ${toAdd.length} member${toAdd.length !== 1 ? 's' : ''} as ${ROLE_LABEL[roleVal]}`
          );
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      list = list.filter(
        (u) => u.cgmp_userprincipalname?.toLowerCase().includes(q) || u.owneridname?.toLowerCase().includes(q)
      );
    }
  ...
  const doBulkAssign = useCallback(async () => {
    ...
    for (const id of selectedIds) {
      ...
      try {
        const r = await Cgmp_userprofilesService.update(id, {
  ...
  /* G2-12: Security Events tab — fetch audit logs for security-related events */
  const [securityEvents, setSecurityEvents] = useState<Cgmp_auditlogsBase[]>([]);
  const [eventsLoading, setEventsLoading] = useState(false);

  useEffect(() => {
    if (secTab !== 'events') return;
    setEventsLoading(true);
    Cgmp_auditlogsService.getAll({
      top: 100,
      orderBy: ['createdon desc'],
    } as never)
      .then((r: { data?: Cgmp_auditlogsBase[] }) => {
        const logs = r.data ?? [];
        const filtered = logs.filter((log: Cgmp_auditlogsBase) =>
          [100000003, 100000006, 100000007, 100000008].includes((log as any).cgmp_eventtype as number)
        );
  ...
  const saveRole = async (userId: string, skipPrivilegeCheck = false) => {
    ...
    if (!skipPrivilegeCheck && isHighPrivilege && newRole !== prevRole) {
      const ownedChanges = changes.filter((c) => c.owneridname === targetUser?.owneridname).length;
      const msg =
        ownedChanges > 0
    ...
    try {
      ...
      Cgmp_auditlogsService.create({
        ...
        cgmp_newvalues: JSON.stringify({
          role: ROLE_LABEL[newRole] ?? String(newRole),
          changedBy: currentUserName,
          browser: getBrowserInfo(),
        }),
      } as unknown as Omit<Cgmp_auditlogsBase, 'cgmp_auditlogid'>).catch((err) => {
    ...
    return (
      <div className="module-workspace">
        ...
        <div className="access-denied">
          ...
          <p className="access-denied__msg">
            This section is restricted to platform administrators. Contact your admin to request access.
          </p>
        </div>
      </div>
    );
    ...
    <div className="module-workspace">
      <div className="module-header">
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="btn btn--outline btn--sm" onClick={() => exportRoleMatrix()} disabled={users.length === 0}>
            Export Matrix
          </button>
      ...
      <div className="ism-tabs" role="tablist" aria-label="Security management views" style={{ padding: '0 24px' }}>
        <button
          ...
          onKeyDown={(e) => {
            if (e.key === 'ArrowRight') {
              e.preventDefault();
              handleSecTabChange('activity');
              document.getElementById('secroles-tab-activity')?.focus();
            } else if (e.key === 'ArrowLeft') {
              e.preventDefault();
              handleSecTabChange('events');
              document.getElementById('secroles-tab-events')?.focus();
            }
          }}
        >
        ...
        <button
          ...
          onKeyDown={(e) => {
            if (e.key === 'ArrowRight') {
              e.preventDefault();
              handleSecTabChange('itopspocs');
              document.getElementById('secroles-tab-itopspocs')?.focus();
            } else if (e.key === 'ArrowLeft') {
              e.preventDefault();
              handleSecTabChange('roles');
              document.getElementById('secroles-tab-roles')?.focus();
            }
          }}
        >
          Session Activity
        ...
        <button
          role="tab"
          id="secroles-tab-events"
          aria-selected={secTab === 'events'}
          aria-controls="secroles-panel-events"
          tabIndex={secTab === 'events' ? 0 : -1}
          className={`ism-tab${secTab === 'events' ? ' ism-tab--active' : ''}`}
          onClick={() => handleSecTabChange('events')}
          onKeyDown={(e) => {
            if (e.key === 'ArrowRight') {
              e.preventDefault();
              handleSecTabChange('roles');
        ...
        <div id="secroles-panel-roles" role="tabpanel" aria-labelledby="secroles-tab-roles">
          ...
          {selectedIds.size > 0 && (
            <div className="sr-bulk-bar">
              <span className="sr-bulk-bar__count">
              ...
              <button
                ...
                onClick={() => setBulkAssignConfirm(true)}
              ...
              <table className="ism-table">
                ...
                <tbody>
                  {filtered.map((u) => {
                    ...
                    const projCount = u.cgmp_assignedprojectids
                      ? u.cgmp_assignedprojectids.split(',').filter((s) => s.trim()).length
                      : 0;
                    return (
                      <tr key={u.cgmp_userprofileid}>
                        ...
                        <td>
                          {(() => {
                            ...
                            return (
                              <span
                                className={`badge badge--status ${isActive ? 'status-released' : 'status-cancelled'}`}
                              >
                          ...
                          {isEditing ? (
                            <div style={{ display: 'flex', gap: 4 }}>
                              <button
                                ...
                                onClick={() => saveRole(u.cgmp_userprofileid)}
                                disabled={saving}
                              >
                                Save
                              </button>
                              <button className="btn btn--xs btn--outline" onClick={cancelEdit} disabled={saving}>
                                Cancel
                              </button>
                            </div>
                          ) : (
                            <div style={{ display: 'flex', gap: 4 }}>
                              <button className="btn btn--xs btn--outline" onClick={() => startEdit(u)}>
                                Edit Role
                              </button>
                              <button
      ...
      {secTab === 'activity' && (
        <div
          ...
          id="secroles-panel-activity"
          role="tabpanel"
          aria-labelledby="secroles-tab-activity"
        >
          {loading ? (
                    ...
                    return (
                      <tr key={u.cgmp_userprofileid}>
                        <td style={{ fontWeight: 500 }}>
                          {deriveName(u.cgmp_userprincipalname, u.owneridname, sysUserMap)}
                        </td>
                        <td style={{ fontSize: 12, fontFamily: 'monospace' }}>{u.cgmp_userprincipalname ?? '—'}</td>
                        <td>
                          <span className={`badge badge--status ${ROLE_CLASS[rc] ?? 'status-draft'}`}>
                            {ROLE_LABEL[rc] ?? 'Unknown'}
                          </span>
                        </td>
      ...
      {secTab === 'itopspocs' && (
        <div
          className="module-table-wrap"
          id="secroles-panel-itopspocs"
          role="tabpanel"
          aria-labelledby="secroles-tab-itopspocs"
        >
          {loading ? (
            ...
            <table className="ism-table">
              ...
              <tbody>
                {itOpsPocProfiles.map((p) => {
                  const assignedLocs = ((p as any).cgmp_assignedlocations ?? '')
                    .split(',')
                    .map((l: string) => l.trim())
                    .filter(Boolean);
                        ...
                        {isEditing ? (
                          <div>
                            ...
                            <div style={{ display: 'flex', gap: 6 }}>
                              ...
                              <button className="btn btn--outline btn--sm" onClick={() => setEditingPocId(null)}>
                                Cancel
                              </button>
                            </div>
                          </div>
                          ...
                          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                            {assignedLocs.length > 0 ? (
                              assignedLocs.map((loc: string) => (
                                <span key={loc} className="badge badge--status">
                                  {loc}
                                </span>
                              ))
                            ) : (
      ...
      {secTab === 'events' && (
        <div
          className="module-table-wrap"
          id="secroles-panel-events"
          role="tabpanel"
          aria-labelledby="secroles-tab-events"
        >
          {eventsLoading ? (
            <div className="module-loading">Loading security events…</div>
          ) : securityEvents.length === 0 ? (
            <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-tertiary)' }}>
              No security events found in the last 100 audit log entries.
            </div>
          ) : (
            <table className="ism-table">
              <thead>
              ...
              <tbody>
                {securityEvents.map((evt, idx) => {
                  const evtType = (evt as any).cgmp_eventtype as number;
                  const evtLabel =
                    evtType === 100000006
                      ? 'Role Changed'
                      : evtType === 100000007
                  ...
                  const evtColor =
                    evtType === 100000007
                      ...
                      : evtType === 100000008
                        ? 'var(--orange)'
                        : 'var(--text-primary)';
                  let details = (evt as any).cgmp_newvalues ?? '';
                  try {
                    details = JSON.stringify(JSON.parse(details), null, 0).slice(0, 120);
                  } catch {}
                  return (
                    <tr key={(evt as any).cgmp_auditlogid ?? idx}>
                      <td style={{ fontSize: 12, whiteSpace: 'nowrap' }}>
                        {(evt as any).createdon ? fmtDateTime((evt as any).createdon) : '—'}
                      </td>
                      <td style={{ fontSize: 12 }}>
                        {(evt as any).cgmp_username ?? (evt as any).cgmp_useremail ?? '—'}
                      </td>
                      <td>
                        <span style={{ fontSize: 12, fontWeight: 600, color: evtColor }}>{evtLabel}</span>
                      </td>
                      <td
                        style={{
                          fontSize: 11,
                      ...
                      >
                        {details || (evt as any).cgmp_auditlogname || '—'}
                      </td>
                    </tr>
                  );
                })}
      ...
      <Dialog
        ...
        footer={
          <div style={{ display: 'flex', justifyContent: 'space-between', width: '100%', gap: 8 }}>
            <button className="btn btn--outline btn--sm" onClick={() => setAddOpen(false)}>
              Cancel
            </button>
            <button
              className="btn btn--primary btn--sm"
              ...
              disabled={
                addSaving ||
                (addMode === 'individual' ? !addSelUser : resolvedMembers.filter((m) => m.user).length === 0)
              }
            >
              {addSaving
                ? 'Adding…'
                : addMode === 'individual'
                  ? addSelUser
                    ? `Add ${addSelUser.fullname ?? 'User'} as ${ROLE_LABEL[parseInt(addRole)]}`
                    : 'Add User'
      ...
      >
        ...
        <div style={{ marginBottom: 16 }}>
          <label
            style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', display: 'block', marginBottom: 4 }}
          >
            {addMode === 'group' ? 'Role for all members' : 'Role'}
          </label>
          <select className="ff-input ff-select" value={addRole} onChange={(e) => setAddRole(e.target.value)}>
            {ROLE_OPTS.map((o) => (
              <option key={o.value} value={o.value}>
                {o.label}
              </option>
                  ...
                  >
                    {addSearchResults.length === 0 ? (
                      <div style={{ padding: '10px 12px', fontSize: 13, color: 'var(--text-tertiary)' }}>
                        No users match "{addSearch}"
                      </div>
                    ) : (
                      addSearchResults.map((u) => {
                        const alreadyHasProfile = users.some(
                          (p) =>
                            p.cgmp_userprincipalname?.toLowerCase() ===
                            (u.internalemailaddress || u.domainname || '').toLowerCase()
                        );
                        return (
                          <button
                            ...
                            style={{
                              ...
                              background: 'none',
                              border: 'none',
                              cursor: alreadyHasProfile ? 'not-allowed' : 'pointer',
                              textAlign: 'left',
                              opacity: alreadyHasProfile ? 0.5 : 1,
                            }}
                            onClick={() => {
                          ...
                          >
                            ...
                            <span style={{ flex: 1, minWidth: 0 }}>
                              ...
                              >
                                {u.internalemailaddress || u.domainname}
                              </span>
                            </span>
                            {alreadyHasProfile && (
          ...
          <div>
            ...
            <div style={{ marginBottom: 10 }}>
              ...
              <textarea
                ...
                value={addGroupUpns}
                onChange={(e) => {
                  setAddGroupUpns(e.target.value);
                  setResolvedMembers([]);
                }}
                  ...
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 12px' }}>
                    {m.user ? (
                      <>
                        ...
                        <span style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <span style={{ display: 'block', fontSize: 11, color: 'var(--text-secondary)' }}>
                            {m.user.internalemailaddress || m.user.domainname}
                          </span>
                        </span>
                        {users.some(
                          (p) =>
                            p.cgmp_userprincipalname?.toLowerCase() ===
                            (m.user!.internalemailaddress || m.user!.domainname || '').toLowerCase()
                        ) && (
                          <span style={{ fontSize: 10, color: 'var(--text-tertiary)', flexShrink: 0 }}>
                            Already added
                          </span>
                      ...
                      <>
                        ...
                        <span style={{ fontSize: 11, color: 'var(--danger)', marginLeft: 'auto' }}>
                          Not found in O365
                        </span>
                      </>
                    )}
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          ...
          if (t) doToggleActive(t);
        }}
        title={toggleTarget && (toggleTarget as any).cgmp_isactive !== false ? 'Deactivate User' : 'Activate User'}
        message={
          toggleTarget
            ? `${(toggleTarget as any).cgmp_isactive !== false ? 'Deactivate' : 'Activate'} ${toggleTarget.owneridname ?? toggleTarget.cgmp_userprincipalname}?`
            : ''
        }
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          const r = saveRoleConfirm;
          setSaveRoleConfirm(null);
          if (r) saveRole(r.userId, true);
        }}
